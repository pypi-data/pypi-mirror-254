...
def generate_listing_dataframe(columns, df_meta, df_preAmz):
    ...
    # 基本
    df_amazon['feed_product_type'] =  df_preAmz['feed_product_type'][0]
    df_amazon['item_sku'] = [parent_sku] + [parent_sku + f'{i:03}' for i in range(1, sku_qty + 1)]
    df_amazon['update_delete'] = df_preAmz['update_delete'][0]
    df_amazon['brand_name'] = df_preAmz['brand_name'][0]
    df_amazon['item_type'] = df_preAmz['item_type'][0]

    df_amazon['quantity'] = df_preAmz['quantity'][0]
    df_amazon.loc[0,'quantity'] = np.nan
    df_amazon['standard_price'] = df_preAmz['standard_price'][0]
    df_amazon.loc[0,'standard_price'] = np.nan
    df_amazon['age_range_description'] = df_preAmz['age_range_description'][0]
    df_amazon['target_gender'] = df_preAmz['target_gender'][0]
    ...
    # 变体信息
    df_amazon['relationship_type'] = df_preAmz['relationship_type'][0]
    df_amazon.loc[0,'relationship_type'] = np.nan
    df_amazon['variation_theme'] = df_preAmz['variation_theme'][0]
    df_amazon['parent_sku'] = df_preAmz['item_sku'][0]
    df_amazon.loc[0,'parent_sku'] = np.nan
    df_amazon['parent_child'] = df_preAmz['parent_child'][0]
    df_amazon.loc[0,'parent_child'] = 'Parent'

    # 商品发现信息
    df_amazon['metal_type'] = df_preAmz['metal_type'][0]
    df_amazon['material_type'] = df_preAmz['material_type'][0]
    df_amazon['material_type'] = df_preAmz['material_type'][0]
    df_amazon['bullet_point1'] = df_preAmz['bullet_point1'][0]
    df_amazon['bullet_point2'] = df_preAmz['bullet_point2'][0]
    df_amazon['bullet_point3'] = df_preAmz['bullet_point3'][0]
    df_amazon['bullet_point4'] = df_preAmz['bullet_point4'][0]
    df_amazon['bullet_point5'] = df_preAmz['bullet_point5'][0]
    df_amazon['generic_keywords'] = df_preAmz['generic_keywords'][0]

    df_amazon['color_name'] = [np.nan] + [colors_en[(i - 1) % colors_en.__len__()] for i in range(1, sku_qty + 1)]
    df_amazon['department_name'] = df_preAmz['department_name'][0]
    df_amazon['size_name'] = [np.nan] + [sizes_en[math.floor(((i - 1) / colors_en.__len__()))] for i in range(1, sku_qty + 1)]
    df_amazon['lifecycle_supply_type'] = df_preAmz['lifecycle_supply_type'][0]
    df_amazon['gem_type'] = df_preAmz['gem_type'][0]


    df_amazon['list_price'] = df_preAmz['list_price'][0]
    df_amazon.loc[0,'list_price'] = np.nan
    df_amazon['item_package_quantity'] = df_preAmz['item_package_quantity'][0]
    df_amazon.loc[0,'item_package_quantity'] = np.nan
    df_amazon['merchant_shipping_group_name'] = df_preAmz['merchant_shipping_group_name'][0]
    df_amazon.loc[0,'merchant_shipping_group_name'] = np.nan
    df_amazon['condition_type'] = df_preAmz['condition_type'][0]
    df_amazon.loc[0,'condition_type'] = np.nan
    df_amazon['sale_price'] = df_preAmz['sale_price'][0]
    df_amazon.loc[0,'sale_price'] = np.nan
    df_amazon['sale_from_date'] = df_preAmz['sale_from_date'][0]
    df_amazon.loc[0,'sale_from_date'] = np.nan
    df_amazon['sale_end_date'] = df_preAmz['sale_end_date'][0]
    df_amazon.loc[0,'sale_from_date'] = np.nan

    assert isinstance(title, str), "title不是字符串"
