...
class TriggerOutArduino:
    ...
    def send_stim_end(self) -> None:
        """Send a trigger for stim end."""
        self._serial.write(STIM_END_COMMAND.encode())
        self.logger.info(f"Stim end -> Command: {STIM_END_COMMAND}")

    def send_stop_trigger(self) -> None:
        """Send a trigger for trial end."""
        self._serial.write(TRIAL_END_COMMAND.encode())
        self.logger.info(f"Stop trigger -> Command: {TRIAL_END_COMMAND}")
