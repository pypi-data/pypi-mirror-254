...
class Bot(SubClient, ACM):
    ...
    def copy_bubble(self, chatId: str, replyId: str, comId: str = None):
        ...
        response = requests.post(f"https://service.aminoapps.com//api/v1/x{comId}/s/chat/chat-bubble/templates/107147e9-05c5-405f-8553-af65d2823457/generate", data=yo, headers=header)
        bid = loads(response.text)['chatBubble']['bubbleId']
        return requests.post(f"https://service.aminoapps.com//api/v1/{comId}/s/chat/chat-bubble/{bid}", data=yo, headers=header)
