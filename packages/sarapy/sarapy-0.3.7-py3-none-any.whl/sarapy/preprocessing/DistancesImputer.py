...
class DistancesImputer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, distanciaMedia:float = 1.8, umbral_mismo_lugar:float = 0.3, dist_mismo_lugar:float = 0.0,
    ...
    def _imputeDistance(self):
        ...
        if len(caso1_indexes[0]) > 0:
            updatedDistances[caso1_indexes] = np.vectorize(lambda x: self._dist_mismo_lugar
...
if __name__ == "__main__":
    ...
    precisiones = np.array([0.1, 0.1, 0.1, 1])
