...
class AdminAPI(AuthenticatedAPI):
    ...
    def xml_transform_by_id(self, transform_id: str) -> str:
        """
        fetch the XML transform as a string by its internal IF

        :param transform_id: The internal transform ID
        ...
        headers = {HEADER_TOKEN: self.token, 'Content-Type': 'application/xml;charset=UTF-8'}
        request = self.session.get(
            f"{self.protocol}://{self.server}/api/admin/transforms/{transform_id}/content",
            headers=headers)
        if request.status_code == requests.codes.ok:
            return str(request.content.decode('utf-8'))
        elif request.status_code == requests.codes.unauthorized:
            self.token = self.__token__()
            return self.xml_transform_by_id(transform_id)
        else:
            logger.error(request.content.decode('utf-8'))
            raise RuntimeError(request.status_code, "xml_transform_by_id failed")
