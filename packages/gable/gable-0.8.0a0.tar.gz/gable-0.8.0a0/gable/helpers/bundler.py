...
def get_project_directory(entry_point):
    return os.path.abspath(os.path.dirname(entry_point))
...
def get_installed_packages(project_dir):
    ...
    for index, package in enumerate(packages, start=1):
        ...
        logger.info(f"\t{index}/{total_packages}: {package['name']}")
...
def create_gzip_bundle_buffer(project_dir, dependencies, debug=False):
    logger.debug("Starting the bundling process...")
    with tempfile.TemporaryDirectory() as temp_dir:
        logger.debug("Copying project files from the root directory...")
        ...
        for dep in dependencies:
            ...
            logger.debug(f"Added dependency: {dep_name}")

        logger.debug("Bundling all the files into a .tar.gz archive in memory...")
        tar_buffer = BytesIO()
        with tarfile.open(fileobj=tar_buffer, mode="w:gz") as tar_file:
            tar_file.add(temp_dir, arcname=os.path.basename(project_dir))
        logger.debug("Bundle creation complete. Resetting buffer position...")
        tar_buffer.seek(0)
        if debug:
            logger.debug("Creating .tar.gz file on disk for debugging...")
            debug_file_path = os.path.join(
                os.getcwd(), f"{os.path.basename(project_dir)}.tar.gz"
            )
            logger.debug("debug_file_path", debug_file_path)
            with open(debug_file_path, "wb") as file:
                file.write(tar_buffer.read())
            tar_buffer.seek(0)
            logger.debug(f"Debug .tar.gz file created at {debug_file_path}")
        logger.debug("Bundling process completed successfully.")
        return tar_buffer


def get_bundled_project(entry_point, debug=False):
    project_dir = get_project_directory(entry_point)
    dependencies = get_installed_packages(project_dir)
    return create_gzip_bundle_buffer(project_dir, dependencies, debug)
