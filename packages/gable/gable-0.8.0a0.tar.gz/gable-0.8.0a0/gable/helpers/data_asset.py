...
def post_ingest_data_asset(
    ...
    def gzip_payload_remover(json_payload):
    ...
    if source_type == "python":
        body["emitterFunction"] = emitter_function
        body["emitterFunctionPayloadParameter"] = emitter_payload_parameter
        body["eventNameKey"] = event_name_key
        body["emitterFilePath"] = emitter_file_path
        if temp_upload_key is not None:
            body["uploadKey"] = temp_upload_key
            body["decryptionKey"] = temp_encryption_key
    result, success, status_code = client.post(
        "v0/data-asset/ingest", json=body, log_payload_filter=gzip_payload_remover
    )
...
# project_name = os.path.basename(os.path.abspath(project_root))
# # Use the provided entry point for the Python project
# debug_mode = ctx.params.get("debug", False)
# gzip_payload = get_bundled_project(project_root, debug_mode)
# if gzip_payload is not None:
#     gzip_payload.seek(0)
#     gzip_payload, temp_encryption_key = encrypt_gzip_payload(gzip_payload)
#     # Get an S3 url to upload the payload to
#     response, success, status_code = ctx.obj.client.get(
...
# schema_contents.append(project_name)
# source_names.append(project_name)
# logger.info(
#     f"{project_name} project bundled successfully, starting static code analysis..."
