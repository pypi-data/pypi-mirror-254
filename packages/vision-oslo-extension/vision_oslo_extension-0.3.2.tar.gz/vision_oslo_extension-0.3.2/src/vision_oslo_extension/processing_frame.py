...
from vision_oslo_extension.shared_contents import SharedVariables, Local_Shared
from vision_oslo_extension.base_frame import BasePage
from vision_oslo_extension import post_processing, batch_processing,average_load,protection_if,grid_connection,ole_processing,dc_tru
...
class P11(BasePage):
    def __init__(self, parent, controller):
        ...
        # add widgets here
        head = tk.Label(master=self.headframe, text = 'Page 11: Substation TRU Assessment Summary',font = controller.sub_title_font)
        head.pack()
        ...
        explain2 = tk.Label(master=self.headframe, text = 'NOTE: Import "Excel - For TRU Summary if needed.',font = controller.text_font)
        explain2.pack()

        option1 = tk.StringVar(value = "0") # Initialize with a value not used by the radio buttons
        choice1 = tk.Radiobutton(master=self.optionframe, text = 'Option 1: Default ', value="1", variable=option1)
        choice1.grid(row = 0, column = 0, sticky = "w", padx=5, pady=5)

        choice2 = tk.Radiobutton(master=self.optionframe, text = 'Option 2: To be developed',value="2", variable=option1)
        choice2.grid(row = 1, column = 0, sticky = "w", padx=5, pady=5)
    ...
    def run_excution_in_thread(self, sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
        try:    
            continue_process = dc_tru.main(sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step)
            if not continue_process:
                # Do something if the process should not continue
                print('\033[1;31m') # color control warning message red
                print("ERROR: Process terminated unexpectly. Please Check Error History Above or Contact Support. You can continue use other options...")
