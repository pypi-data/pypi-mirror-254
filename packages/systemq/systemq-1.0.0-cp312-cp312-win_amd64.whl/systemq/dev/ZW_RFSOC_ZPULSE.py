...
from .common import BaseDriver, QBool, QInteger, QReal, QVector, QString

from waveforms import Waveform
# try:
from .common.ZWD_DDS import soc_zpulse as fpgadev
# except:
...
class Driver(BaseDriver):
    ...
    quants = [
        ...
        QInteger('SamplingMode', ch=1, value=8000),
    ...
    def set_DA(self, name, value, ch):
        ...
        """
        if name in ['Waveform']:
            trigger_delay = round(self.config['TriggerDelay'][ch]['value']*self.srate)
            ...
            if value <= 1 and value>=-1:
                ...
                self.handle.dac_offset(ch-1, np.round(self.handle.dc1v * value + self.handle.offset[ch-1]))
            else:
    ...
    def writeWaveform(self, data, ch=1, trigger_delay=0, continuous=False):
        ...
        self.triggerClose()
        self.handle.dac_updata(ch-1, trigger_delay, replay_cnt, replay_continue_flag, dac_data)
