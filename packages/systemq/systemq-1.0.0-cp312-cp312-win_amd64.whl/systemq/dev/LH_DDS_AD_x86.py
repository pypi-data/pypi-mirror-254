...
from .common import (BaseDriver, QBool, QInteger, QList, QReal, QString,
                     QVector, get_coef)
from vios.driver.common.quantity import newcfg

try:
    from .common.DDS_LH import ddsx86_ad
except:
...
class Driver(BaseDriver):
    ...
    def write(self, name: str, value, **kw):
        ...
        if name in [
            ...
            assert len(f_list) in range(1, 20), f'Invalid coefficient number of frequencies.'
            ...
            if value.get('wList', {}).get('phi', None) is not None:
                ...
                self.threshold[ch] = np.array([kw['threshold'] for kw in value['wList']])
        elif name in ['SamplingMode']:
            if value in ['4G']:
                if self._sampling_mode in [None, 0]:
                    self._sampling_mode = 1
                    self.handle.system_sampling(sampling=self._sampling_mode,step=0,self_set=1)
                    self.srate = 2e9
                    time.sleep(1)
            elif value in ['5G']:
    ...
    def read(self, name: str, **kw):
        ...
        if name in ['TraceIQ']:
            ...
            avg = self.config['avg'][ch]['value']
            classfy = self.config['Classify'][ch]['value']
            data = self.get_IQ_alg(avg=avg or ((classfy & 146)==146), timeout=None, ch=ch)
            if (classfy & 132)==132:
                state = ((data[0]+1j*data[1]) * np.exp(-1j * self.phi[ch])).real > self.threshold[ch]
                if (classfy & 196) == 196:
            ...
            order = [int(item[2:]) for item in sorted([f'CH{i}' for i in self._data_cache.keys()])]
            ret = {}
            for i in range(shots):
                count_state = np.concatenate(tuple(self._data_cache[j][i] for j in order))
