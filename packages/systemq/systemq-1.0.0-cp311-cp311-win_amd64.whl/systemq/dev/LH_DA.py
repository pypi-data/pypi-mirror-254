...
from .common import BaseDriver, QOption, QReal, QVector
...
class Driver(BaseDriver):
    ...
    def write(self, name: str, value, **kw):
        ...
        #     self.Trig(500,num,continueflag=False)
            ...
            #     value = self.dist(*value)
                ...
                print('type error')
                return
            assert len(np.nonzero(value)[0]) <= int(50e-6*self.srate),'Wave is too long!'
    ...
    def Trig(self,triggertime,times=2048,continueflag=True,trigger_source=False,slot='slot6'):
        ...
        '''
        self.handle.settrigT(triggertime,times,continueflag,trigger_source,slot)
    ...
    def writeWaveform(self,data,ch=1,continuous=False):
        data_point_ch = [{'wave_id':0,'amp':1,'repeat_num':1,'continuous':continuous}]
        setting_save =self.handle.wr_dac_data_single_ch([data*(2**15-1)],ch,self.addr)
        setting_save1 = self.handle.wr_dac_setting_single_ch(data_point_ch,setting_save,ch,self.addr)

    def writeMarker(self,data,ch=1,marker_delay=0):
        self.handle.close_all_marker('slot6')

        if ch in list(range(1,17)):
            marker_setting = self.handle.wr_marker_singlechennel(ch-1,[data],'slot6')
            marker_data_point = []
            marker_data_point.append({'marker_trigger_delay':marker_delay,'marker_num':0,'marker_repeat_num':1})
            self.handle.wr_marker_single_setting(ch-1,marker_data_point,marker_setting,'slot6')
        else:
            print('wrong data type or wrong ch number, must be in [1-16].')
        self.handle.open_marker('slot6')

    def beforeSet(self):
        self.handle.settrigT(self.default_triggertime,1,continueflag=0,trigger_source=0,slot='slot6')
        # time.sleep(0.1)

    def afterSet(self):
        # print('wait')
        # time.sleep(0.1)
        self.handle.settrigT(self.default_triggertime,1,continueflag=1,trigger_source=self.trigger_source,slot='slot6')
