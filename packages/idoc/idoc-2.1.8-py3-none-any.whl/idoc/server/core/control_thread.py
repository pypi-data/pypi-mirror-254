...
from idoc.server.core.base import Base, Status
...
from idoc.helpers import hours_minutes_seconds, iso_format
...
class ControlThread(Base):
    ...
    def __init__(self, machine_id, version, result_dir, user_data, *args, **kwargs):
        ...
        if user_data["adaptation_time"] is None:
            adaptation_time = config.content["experiment"]["adaptation_time"]
        else:
        ...
        self._location = config.content['experiment']['location']
        self._max_duration = user_data['max_duration'] or config.content['experiment']['max_duration']
    ...
    def _pick_class(self, category):
        ...
        class_name = self._user_classes[category] or config.content['default_class'][category]
    ...
    def initialise_result_writer(self):
        ...
        result_writer_args = config.content['io']['result_writer']['args']
        result_writer_kwargs = config.content['io']['result_writer']['kwargs']
    ...
    def initialise_recognizer(self):
        ...
        roi_builder_args = config.content['roi_builder']['args']
        roi_builder_kwargs = config.content['roi_builder']['kwargs']
        ...
        drawer_kwargs = config.content['drawer']['kwargs']
        ...
        recognizer = Recognizer(
            tracker_class, camera_class, roi_builder, self.result_writer, drawer_class=drawer_class,
            drawer_kwargs=drawer_kwargs,  user_data=self._user_data
        )
    ...
    def warm_up(self):
        ...
        """
        if self._warmed_up:
            logger.debug("You already warmed up. Warming up again is disabled")
        else:
            self._warmed_up = True
            self.controller.run_minimal()
            logger.debug("Preparing recognizer")
            self.recognizer.prepare()
            logger.debug("Starting recognizer")
        ...
        if self.recognize:
            ...
            if not self.recognizer.ready:
                try:
                    ...
                    if not self._warmed_up: self.recognizer.prepare()
                except Exception as error:
