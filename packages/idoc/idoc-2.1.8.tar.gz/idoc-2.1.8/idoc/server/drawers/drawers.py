...
try:
    from cv2.cv import CV_FOURCC as VideoWriter_fourcc # type: ignore
...
class BaseDrawer(Base):
    ...
    def last_drawn_path(self):
        return config.content["drawer"]["last_drawn_path"]

    @property
    def last_annot_path(self):
        return config.content["drawer"]["last_annot_path"]
    ...
    def write_videos(self, video_root='video.avi'):
        ...
        if len(self._video_writers) == 0:
            ...
            logger.debug(f"Output video framerate: {framerate}")
