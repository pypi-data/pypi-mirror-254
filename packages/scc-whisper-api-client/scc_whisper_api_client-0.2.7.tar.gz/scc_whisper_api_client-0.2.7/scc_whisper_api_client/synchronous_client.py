...
class SCCSyncClient(SCCClient):
    ...
    def _sync_wrapper(
        ...
        if max_wait_time is None:
            ...
            max_tries = max_wait_time // config.SYNC_RETRIES_PAUSE_TIME
        task_res = add_func()
        ...
        if task_res.task_id:
            ...
            while c <= max_tries:
                sleep(
                    1 + c * config.SYNC_RETRIES_PAUSE_TIME
                ...
                if res.state in ["SUCCESS"]:
                    return res.result
                elif res.state in ["FAILURE"]:
                    raise Exception(
                        f"Task {task_res.task_id} failed with error: {res.result.error if res.result.error else 'unknown'}"
                    )
                else:
                    logger.info(
