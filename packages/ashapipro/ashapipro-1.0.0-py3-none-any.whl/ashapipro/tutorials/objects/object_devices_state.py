...
class ObjectDevicesStateTask(SimcomplexTask):
    ...
    def on_scene_opened(self, response):
        ...
        print(f"Opened scene contains {ship.code} object, with following states for propulsion-steering system:")
        for tag in ("propeller.port", "propeller.stbd", "engine.port", "engine.stbd", "rudder.port", "rudder.stbd"):
            device = ship.devices[tag]
            print(f'{tag}')
            for pname, pvalue in device.states.items():
...
if __name__ == '__main__':
    ...
    with local_server(config):
        ...
        task = ObjectDevicesStateTask(config, "api/objects/cargo01_full_speed_ahead_01.stexc", "cargo01")
