...
def mutual_lookup(lmax: int, positions_1: np.ndarray, positions_2: np.ndarray, refractive_index: np.ndarray, derivatives: bool = False, parallel: bool = False):
    ...
    if parallel:
        ...
        spherical_bessel, spherical_hankel, e_j_dm_phi, p_lm = compute_lookup_tables(lmax, size_parameter, phi, cosine_theta)
    else:
        ...
        if derivatives:
            spherical_hankel_lower = np.sqrt(np.divide(np.pi / 2, size_parameter_extended, out = np.zeros_like(size_parameter_extended), where = size_parameter_extended != 0)) * hankel1(-1/2, size_parameter_extended)
            spherical_hankel_lower = np.vstack((spherical_hankel_lower, spherical_hankel[:-1, :, :, :]))
            spherical_hankel_derivative = size_parameter_extended * spherical_hankel_lower - p_range * spherical_hankel
            ...
            for p in range(2 * lmax + 1):
                for absdm in range(p + 1):
                    cml = np.sqrt((2 * p + 1) / 2 * np.prod(1 / np.arange(p - absdm + 1, p + absdm + 1)))
                    # if np.isnan(cml):
                    #     print(p)
                    #     print(absdm)
...
def material_handler(links):
    ...
    data = dict(
        ref_idx=pd.DataFrame(columns=['wavelength', 'n', 'k']),
        material=None
    )
    for link in links:
        link = link.strip()
        ...
        for line in data_yml['DATA']:
            ...
            #   header_yml=['wavelength', 'n']
                ...
                if line['type'].lower() == 'formula 1':
                    ...
                    coefficients = np.array([float(c) for c in line['coefficients'].split()])
                    ref_idx = lambda x: np.sqrt(1 + np.sum([coefficients[i] * x**2 / (x**2 - coefficients[i+1]**2) for i in range(1, len(coefficients), 2)], axis=0))
...
def handle_eodg(url):
    ...
    with urllib.request.urlopen(req) as resp:
        ...
        data_format = [s.lower() for s in re.search(r'#FORMAT=(.*)\n', data).group(1).split()]
        header_yml=['wavelength', 'n', 'k']
...
def handle_csv(path):
    ...
    data = pd.read_csv(path, delim_whitespace=False, header=0, names=['wavelength', 'n', 'k'])
    return data, material
