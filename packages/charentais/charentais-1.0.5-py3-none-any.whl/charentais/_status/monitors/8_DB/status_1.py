...
def check_1 ():
	...
	import charentais.db as charentais_db
	records_1 = charentais_db.records (
		db_directory = normpath (join (variable, f"status_db"))
	)

	import charentais
	scan = charentais.start (
	...
	records_2 = charentais_db.records (
		db_directory = normpath (join (variable, f"status_db"))
	)
	assert (len (records_2) == (len (records_1) + 1))
	
	last_record = charentais_db.last_record (
		db_directory = normpath (join (variable, f"status_db"))
	)
