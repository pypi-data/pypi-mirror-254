...
class AsyncScheduler(BaseScheduler):
    ...
    async def enqueue_request(self, request: Request) -> bool:
        ...
        assert self.stats is not None
        if dqok:
            self.stats.inc_value("scheduler/enqueued/persistent", spider=self.spider)
        self.stats.inc_value("scheduler/enqueued", spider=self.spider)
        return True
    ...
    async def next_request(self) -> Optional[Request]:
        ...
        if request is not None:
            self.stats.inc_value("scheduler/dequeued/persistent", spider=self.spider)
        if request is not None:
            self.stats.inc_value("scheduler/dequeued", spider=self.spider)
        return request
    ...
    async def _dqpush(self, request: Request) -> bool:
        ...
        try:
            # await
            await self.dqs.push(request, spider=self.spider)
        except ValueError as e:  # non serializable request
            if self.logunser:
                msg = (
                    "Unable to serialize request: %(request)s - reason:"
    ...
    async def _dqpop(self) -> Optional[Request]:
        if self.dqs is not None:
            # await
            return await self.dqs.pop(spider=self.spider)
        return None
