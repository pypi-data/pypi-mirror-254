...
memtables = {
    'quote_val_mem': {
        ...
        'To': 'quote_val_mem',
        'accounts': ['wind_code', 'trade_dt', 's_dq_freeturnover','s_dq_mv', 'opdate'],
        'dtypes': ['int', 'date', 'double','double','datetime'],
...
def getCashDivdend(conn, codelist: list, startDate: dt.datetime, endDate: dt.datetime, byPeriod=False) -> pd.DataFrame:
    ...
    return d.dropna(subset=['totalAmount'])
...
def getStockWorth(conn, codelist: list, startDate: dt.datetime, endDate: dt.datetime,
    ...
    SELECT
        ...
        trade_dt date,
        s_dq_mv currentWorth,
        opdate{",s_val_mv totalWorth,s_val_pe_ttm PETTM,s_val_ps_ttm PSTTM" if memtableName is None else ""}
    FROM
...
# 取个股上市日期
def getNCode(conn, endDate: dt.datetime) -> list:
    # 返回值 非次新股列表
    a = pd.read_sql(f'''
    SELECT
        f4_0001 as code,
...
def get_TTM_data(conn, start_date:dt.datetime, end_date:dt.datetime, hfile=sys.stdout):
    print(f'TTM数据获取开始时间：', dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),file=hfile)
    ...
    print(f'TTM数据获取结束时间：', dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),file=hfile)
    return TTM_data
...
if __name__ =='__main__':
    ...
    fillDate(conn)
    cleanMemTables(conn, ['finance','finance'],['quote_mem','quote_mem'])
