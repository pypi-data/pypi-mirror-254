...
async def main():
    ...
    try:
        ...
        print(f'Could not load drama: {e}')
    except DramatistParseException as e:
        print(f'Could not parse drama: {e}')
    else:
        if drama:
            if output_folder_path:
                result = json.dumps(drama, default=__json_encoder_drama)
                with open(join(output_folder_path, f'{drama.title}.json'), 'w', encoding='utf-8') as output_file:
                    output_file.write(result)
            else:
                print(drama)
        else:
            print('Could not load drama')
