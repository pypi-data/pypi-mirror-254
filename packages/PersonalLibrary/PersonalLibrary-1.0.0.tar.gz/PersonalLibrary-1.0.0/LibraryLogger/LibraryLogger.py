import datetime
import os
from typing import Final, Literal, Self, Any
...
class LibraryLogger:
    ...
    def _toFile(self, file: str, mode: Literal['r', 'a', 'w'] = 'r'):
        try:  # 尝试打开文件
            with open(file + '.log', mode, encoding='utf-8') as files:
                if mode == 'r':
                    if files.read() == "" or files.read() == "\n":  # 文件内容为空
                        self._toFile(file, 'w')  # 创建文件
                    else:  # 文件内容不为空，追加内容
                        self._toFile(file, 'a')
                elif mode == 'a':
                    logList = self.cacheLog[self.logTime().replace('/', '-').split(" ")[0]]
                    for context in logList:
                        if logList.index(context) != len(logList):
                            files.write(context + "\n")
                        else:
                            files.write(context)
                    files.close()
                    ...
                    self._toFile(file, 'a')
        except FileNotFoundError:  # 不存在则创建
            self._toFile(file, 'w')
    ...
    def tempShow(self, func: callable(Self)):
        self.showLog = True
        func()
        self.showLog = False
        return None
