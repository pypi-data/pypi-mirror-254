...
def presign_upload_file(data:bytes, name:str,resource_id:str, timeout:float|Tuple[float, float])->PresignResult:
  ...
  },headers=auth.get_file_auth_header(), timeout=_get_timeout(timeout))
...
def delete(file_id:str,resource_id:str, timeout:float|Tuple[float, float]=10)->None:
  """
  删除文件
  """
  url = _get_delete_url(resource_id,file_id)
  resp = _get_baas_session().delete(url, headers=auth.get_file_auth_header(), timeout=_get_timeout(timeout))
  return utils._parse_response(resp)

def get_file_info(file_id:str,resource_id:str, timeout:float|Tuple[float, float])->File:
  """
  通过 file_id 下载文件
  """
  url = _get_get_url(resource_id, file_id)
  resp = _get_baas_session().get(url, headers=auth.get_file_auth_header(), timeout=_get_timeout(timeout))
