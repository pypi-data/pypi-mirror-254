...
def create_component(name: str, func: callable, namespace: str = 'default',
                     description: str = '', output_names: list[str] | None = None) -> Component | None:
    ...
    req_body = ExistComponent(PRJ_ID=client_info.project, CMPNT_NM=name, CMPNT_TYPE_CD=PipelineType.experiment_pipeline)
    ...
    return Component(name=name, func=func, namespace=namespace, desc=description, output_names=output_names,
                     component_type=PipelineType.experiment_pipeline)
...
def get_component(name: str, revision: int | None = None):
    ...
    req_body = GetComponent(PRJ_ID=client_info.project, CMPNT_NM=name, CMPNT_TYPE_CD=PipelineType.experiment_pipeline,
                            CMPNT_RVSN=revision)
    code, msg = req_util.post(url=url, data=req_body.dict())
    if code != 0:
    ...
    try:
        ...
        return Component(name=name, func=func, namespace=namespace, desc=desc, output_names=out_names,
