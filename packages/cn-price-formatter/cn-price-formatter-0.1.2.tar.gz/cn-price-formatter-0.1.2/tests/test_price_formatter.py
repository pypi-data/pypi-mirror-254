...
def test_initialization():
    ...
    assert formatter.total_price_before_tax == 0
    assert formatter.total_price_after_tax == 0
    ...
    assert formatter.cn_fee == 6

def test_hotel_planner_valid_json(sample_json_data):
    formatter = PriceFormatter()
    formatter.hotel_planner(sample_json_data)
    assert formatter.total_price_before_tax == 341.97
    assert formatter.total_price_after_tax == 444.41
...
def test_get_total_price_before_tax():
    formatter = PriceFormatter()
    formatter.total_price_before_tax = 341.97
    assert formatter.get_total_price_before_tax() == 341.97