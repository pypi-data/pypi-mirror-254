...
pub struct KoloProfiler {
    ...
    timeout: usize,
    use_threading: bool,
    sql_filter: RefCell<filters::SQLFilter>,
}
...
impl KoloProfiler {
    pub fn new_from_python(py: Python, py_profiler: &PyAny) -> Result<Self, PyErr> {
        ...
        Ok(Self {
            ...
            timeout,
            use_threading,
            sql_filter: filters::SQLFilter::new().into(),
        })
    }
    ...
    fn process_default_include_frames(
        ...
        let filter = match name {
            ...
            "execute_sql" => {
                if self
                    .sql_filter
                    .borrow_mut()
                    .use_sql_filter(filename, py, pyframe)?
                {
                    let call_frames = self.call_frames.get_or_default().borrow().clone();
                    let data =
                        self.sql_filter
                            .borrow()
                            .process(frame, event, arg, call_frames, py)?;
        ...
        let data = utils::dump_msgpack(py, data)?;
        self.push_frame_data(py, data)?;
