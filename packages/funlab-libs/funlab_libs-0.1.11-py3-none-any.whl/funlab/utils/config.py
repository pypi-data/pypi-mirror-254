...
class UpperCaseKeyDict(UserDict):
    """Used for config that make all key as upper case, means case insensitive, and recursively to all dict type vaule"""
...
class Config():
    ...
    def __init__(self, config_file_or_values:str|dict|Config|pathlib.Path, env_file_or_values:str|dict=None,
        ...
        """
        Args:
            config_file_or_values (str | dict | pathlib.Path, optional): check _lookup_config function
            only_section (str, optional): read the specified section. Defaults to None.
            case_insensitive (bool, optional): to get the key's value is case insensitive. Defaults to False.
            env_file_or_values(str|dict, optional): use to decrypt the envrionment variable that specified in configuration.
        Raises:
    ...
    def update_with_default(self, default_conf:Config | dict, section:str=None):
        if isinstance(default_conf, Config):
            default_conf = default_conf.as_dict()

        if section and section in self._raw:
            update_section = self._raw[section]
        else:
            self._raw = default_conf | self._raw
            return self._from_dict(self._raw, only_section=None)

        if section and section in default_conf:
            default_conf = default_conf[section]

        for key in default_conf.keys():
            if not key in update_section:
                update_section[key] = default_conf[key]

        self._from_dict(self._raw, only_section=None)


    def join(self, conf:Config):
        """ Join another config by just adding those keys not exists in this config, and would not update any exists already

        Args:
            config (Config):
        """
        self._raw = conf._raw | self._raw
        self._env_vars = conf._env_vars | self._env_vars
        self._from_dict(self._raw, only_section=None)
    ...
    def get_subsection_config(self, section:str, case_insensitive=False, default=None)->Config:
        try:
            cfg_dict:dict
            if case_insensitive:
...
def main(args=None):
    ...
    scfg = cfg.get_subsection_config('FLASK')
    print(scfg.as_dict())
    scfg = cfg.get_subsection_config('FLASK')
    print(scfg.as_dict())
