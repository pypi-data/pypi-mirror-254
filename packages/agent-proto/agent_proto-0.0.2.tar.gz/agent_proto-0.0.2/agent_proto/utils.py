...
def _clean_dict(d: dict[str, T]) -> dict[str, T]:
    ...
    warnings.warn(
        f"Null values found in the dictionary: {nulls}. They will be removed.",
        UserWarning,
    )
...
def _clean_list(list_: list[T]) -> list[T]:
    ...
    Example:
        ...
        [1, 3]
    """
    nulls: list[T] = [v for v in list_ if v is None]
    warnings.warn(
        f"Null values found in the list: {nulls}. They will be removed.", UserWarning
    )
