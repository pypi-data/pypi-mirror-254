...
class Assistant:
  ...
  def ask(self,user_query:str):
    ...
    if self.rn_status or self.rn_status == 'completed':
  ...
  def run_status(self):
    ...
    while True:
      sleep(3)
      self.rn_status = self.client.beta.threads.runs.retrieve(run_id=self.run.id,thread_id=self.thread.id).status

      print("Status:\t",self.run_status)
      if  self.rn_status =='completed':
        return True
      elif self.rn_status  in ['failed','cancelled','expired']:        
        return False
