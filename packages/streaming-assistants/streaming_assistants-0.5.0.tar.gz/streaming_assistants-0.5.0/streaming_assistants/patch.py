...
from litellm import utils, logging as litellmLogging
    ...
    def patched_create(self, *args, **kwargs):
        ...
        try:
            with contextlib.redirect_stdout(io.StringIO()):
                ...
                if provider == 'bedrock':
                    client._custom_headers['LLM-PARAM-aws-access-key-id'] = os.getenv("AWS_ACCESS_KEY_ID")
                    client._custom_headers['LLM-PARAM-aws-secret-access-key'] = os.getenv("AWS_SECRET_ACCESS_KEY")
                    client._custom_headers['LLM-PARAM-aws-region-name'] = os.getenv("AWS_REGION_NAME")
                if provider != 'openai':
...
def patch(client: Union[OpenAI, AsyncOpenAI]):
    print("Patching OpenAI client, it will now communicate to Astra Assistants API: https://astra.datastax.com/")
    print("Patching methods of `client.beta`")

    if client.base_url is None:
        client.base_url = "https://open-assistant-ai.astra.datastax.com/v1"

    patch_methods(client.beta,client)
    for attr_name in dir(client.beta):
        attr = getattr(client.beta, attr_name)

        if isinstance(attr, FunctionType):
            # Wrap the method and rebind it to client.beta
            patched_method = wrap_method(attr, client)
            setattr(client.beta, attr_name, MethodType(patched_method, client.beta))
    ...
    """
    # Create the patched create method and bind it to the client.beta.assistants instance
    client.beta.assistants.create = MethodType(wrap_create(client.beta.assistants.create, client), client.beta.assistants)
