...
class Client:
    ...
    def get_locations(
        ...
        not_before_ts: Optional[int] = (
            None if not_before is None else math.ceil(not_before.timestamp())
        )

        not_after_ts: Optional[int] = (
            None if not_after is None else math.floor(not_after.timestamp())
        )
        ...
        while max_count > 0:
            ...
            not_after_ts = math.floor(res[-1].datetime.timestamp())
