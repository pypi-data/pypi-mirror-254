from hang-py.ascii_status import *
from random import choice
from hang-py.utils import clear, gen_config, CONFIG
from hang-py.translations import gm
...
def save_points(player: str, points: int):
  ...
  if position > 0:
    ...
    with open('./hang-py/config.ini', 'w') as file_object:
...
def get_words(lang):
  try:
    with open(f"./hang-py/dics/{lang}.txt", "r", encoding="utf-8") as f:
      words = f.readlines()
    if words == []:
