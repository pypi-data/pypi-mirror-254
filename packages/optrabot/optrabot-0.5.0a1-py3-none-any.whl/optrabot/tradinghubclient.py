...
class TradinghubClient():
	...
	async def _placeTakeProfitAndStop(self, entryTrade: Trade):
		...
		now = datetime.now()
		ocaGroup = self._currentStrategy.account + '_' + now.strftime('%H%M%S')
		...
		order.account = self._currentStrategy.account
		...
		stopShortsOrder.account = self._currentStrategy.account
