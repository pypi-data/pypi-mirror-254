...
from optrabot.strategies.strategyfactory import StrategyFactory
...
class Config(UserDict):
	...
	def readStrategies(self):
		self._strategies = []
		for key, data in self.get('strategies').items():
			logger.debug('Reading strategy: {}', key)
			strategy = StrategyFactory.createStrategy(name=key,data=data)
			if strategy != None:
				logger.debug('Added strategy: {}', strategy)
				self._strategies.append(strategy)

	def getStrategies(self) -> List[Strategy]:
		""" Returns the configured strategies
		"""
