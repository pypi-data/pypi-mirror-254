...
from funlab.utils.config import Config
from funlab.utils.dbmgr2 import DbMgr# , DbMgr2
...
class FunlabFlask(_Configuable, Flask):
    def __init__(self, configfile:str, env_file:str, *args, **kwargs):
        ...
        # self.dbmgr: DbMgr = DbMgr(self.myconfig, default_db=self.config.get('DB_USE'))
    ...
    def get_section_config(self, section:str, case_insensitive=True, default=None):
        return self.myconfig.get_subsection_config(section, case_insensitive, default)
    ...
    def _setup_config(self, configfile, env_file:str):
        def setup_flask_env():
        ...
        # app_configfile = Path(inspect.getmodule(self).__file__).parent.joinpath('conf/config.toml')  # APP_DIR.joinpath('conf/config.toml')
        # if app_configfile.exists():
        #     app_default_config = Config(app_configfile, env_file_or_values=env_file)
        # else:
        #     app_default_config : Config = Config({})
        ...
        flask_config = self.get_config('config.toml', group_section='FLASK', ext_config=self.myconfig).as_dict()
        setup_flask_env()
        ...
        def init_plugin_object(plugin_cls:type[ViewPlugin]):
            ...
            if isinstance(plugin, SecurityPlugin):
                if self.login_manager:
                    ...
                    installed_security_plugin = plugin_cls
            ...
            if plugin.login_view and self.login_manager: # has_login_manager:
...
def start_server(app:Flask):
    ...
    if wsgi == 'waitress':
        try:
            ...
            raise Exception("Use waitress as WSGI server, but not found package, please install: pip install waitress") from e
