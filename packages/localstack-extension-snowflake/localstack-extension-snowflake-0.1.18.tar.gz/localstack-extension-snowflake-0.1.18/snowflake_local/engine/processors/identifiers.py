...
from snowflake_local.engine.transform_utils import INTERNAL_IDENTIFIERS,get_canonical_name,is_create_table_expression
...
class ConvertIdentifiersToUpper(QueryProcessor):
	...
	def get_priority(E):from snowflake_local.engine.processors.aliases import AddColumnNamesToTableAliases as A;from snowflake_local.engine.processors.arrays import ConvertArrayFunctionArgTypes as B;from snowflake_local.engine.processors.variants import ReplaceObjectConstruct as C;D=[A().get_priority(),C().get_priority(),B().get_priority(),ReplaceQuestionMarkPlaceholder().get_priority()];return min(D)-1
