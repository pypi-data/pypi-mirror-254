...
def _define_util_functions(database):
	...
	B.run_query('CREATE EXTENSION IF NOT EXISTS plpython3u',database=A);_install_plv8_extension();B.run_query('CREATE EXTENSION IF NOT EXISTS plv8',database=A);D=[];E=';\n'.join(D);B.run_query(E,database=A)
