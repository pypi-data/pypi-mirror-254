...
class lensIQ():
    ...
    def loadData(self, calData) -> str:
        ...
        self.engValues['OD']['min'] = ODMin
        ODMax = self.calData['odMax'] if 'odMax' in self.calData.keys() else 'Inf'
        self.engValues['OD']['max'] = ODMax
        return lensIQ.OK
    ...
    def updateAfterFocus(self, focusStep:int, updateOD:bool=True):
        ...
        if updateOD:
            OD, _err, _ODMin, _ODMax= self.focusStep2OD(focusStep, self.engValues['zoomStep']['value'])
        else:
