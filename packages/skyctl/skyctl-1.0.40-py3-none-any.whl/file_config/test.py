...
@click.option('--space', '-n', help='Namespace for file upload, if namespace is empty, upload to default',
              default='default', required=False)
def file(space):
    if space:
        click.echo("开始上传文件....", space)
    else:
        click.echo("开始上传文件默认....",space)
...
@click.option('--ls', '-l', help='Show the namespace list')
def create_space(create, ls):
    if create:
        click.echo('创建namespace = ', create)
    if ls:
