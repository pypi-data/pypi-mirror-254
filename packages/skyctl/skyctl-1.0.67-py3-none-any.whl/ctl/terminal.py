...
class CustomClickCommand(click.Command):
    def get_help(self, ctx):
        # 可以在这里自定义你的帮助信息
        # 例如，你可以添加一些额外的文本，或者改变默认的格式

        # 调用原始的 get_help 方法来获得默认的帮助信息
        original_help = super(CustomClickCommand, self).get_help(ctx)
        click.echo(f'{original_help}')
        # 在默认帮助信息的基础上添加自定义内容
        custom_help = (
            "This is some custom help text that will appear before the regular help.\n\n"
            f"{original_help}"
            "\n\nAfter the regular help, you can add even more custom text if you want."
        )
...
@login.command('check', help='Check if the skypilot configuration file is effective')
def check():
    user = decryption()
    if authority(user):
