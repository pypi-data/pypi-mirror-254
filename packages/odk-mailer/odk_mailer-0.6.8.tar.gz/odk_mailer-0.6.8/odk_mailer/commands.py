from odk_mailer.lib import parser, prompts, utils, globals, smtp, db, log, cron, notification
...
def evaluate(force=False, dry=False, verbose=False, notify=False):
    ...
    if not dry:        
        if isCronProcess:
            ...
            log.write(f"Evaluate run from cron (HOST: {os.getenv('SMTP_HOST')} PORT: {os.getenv('SMTP_PORT')}) FOO: {os.getenv('FOO')})")
        else: 
    ...
    if total > 0:
        ...
        if confirmed:
            ...
            for idx, eval in enumerate(evaluated):
                ...
                if not dry and notify and success:
