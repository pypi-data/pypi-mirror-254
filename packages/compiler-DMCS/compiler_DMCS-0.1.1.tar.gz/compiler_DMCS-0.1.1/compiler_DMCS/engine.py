...
"""     
  ...
  Don't make up value not in the Input. Output only date and address IF NOT EXISTS return null must be a well-formed JSON object.```json
  ### Input:
...
def endpoint(img:str) -> str:
 try: 
  ...
  ta = dateaddress_detect(rtext(results))
  amap.update(ta)
  check =  mapping.header_map(mapping.nrelocate(results))[1]
  check.append(ta["date"])
  check.append(ta["address"])
  amap.update({"check":check})
  response = {
