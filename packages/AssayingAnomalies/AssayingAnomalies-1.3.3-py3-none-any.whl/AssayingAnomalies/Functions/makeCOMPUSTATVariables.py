...
def makeCOMPUSTATVariables(params, data, quarterly_indicator=False):
    ...
    if not params.remote_or_not:
        ...
        def process_and_save_var(var, ret, quarterly_indicator=False):
            print(f"Now working on COMPUSTAT variable {var}")
            # temptable = pd.pivot_table(crsp_msf, index='permno', columns='dates', values = i)
            temptable = pd.pivot(data, index='dates', columns='permno', values=var)
            # temptable.to_csv(compFolder + col + '.csv')
            ...
            # Print statement
            print(f"Completed COMPUSTAT variable {var} with shape {var.shape}")
