...
def get_hf_spreads_data(params, **kwargs):
    ...
    date_range = np.linspace(params.sample_start, params.sample_end, params.sample_end - params.sample_start + 1).astype(int)
    commands = []
    file_locations = []
...
def make_hf_effective_spreads(params):
    ...
    ret = pd.read_csv(params.crspFolder + '/ret.csv', index_col=0).astype(float)
...
def extend_hf_effective_spreads(params):
    ...
    if params.sample_start <= 2003:
        ...
        pre_2003.columns = pre_2003.columns.astype(float).astype(str)
        ...
        # Filter the pre_2003 data to only the desired columns
        common_cols = pre_2003.columns.intersection(post_2003.columns)
        pre_2003 = pre_2003.loc[:, common_cols] / 100

        # Make sure pre_2003 has the same number of columns
        missing_cols = set(post_2003.columns) - set(pre_2003)
        ...
        # Put columns in pre_2003 in the same order as post_2003
        pre_2003 = pre_2003[post_2003.columns]
