...
def makeCRSPDailyData(params):
    ...
    # If files are .csv then change to .parquet
    # for file in daily_crsp_files:
    #     temp_df = pd.read_csv(file)
    #     parquet_file = file.replace('.csv', '.parquet')
    #     temp_df.to_parquet(parquet_file)
    ...
    # Load the var_names for individual dataframes. The first three col names are unnamed, date, and permno, so we
    # exclude them.
    var_names = pd.read_parquet(daily_crsp_path + 'crsp_dsf_' + str(params.sample_end) + '.parquet').columns[3:].values.flatten()
    ...
    if not params.remote_or_not:
        for i, var in enumerate(var_names):
            ...
            temp_df = temp_df.reindex(index=ddates, columns=permno.values.flatten().astype(str), fill_value=np.nan)
            temp_df.columns = temp_df.columns.astype(str)
            ...
            # test2 = np.intersect1d(permno.values.flatten().astype(str))
