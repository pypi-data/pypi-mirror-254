...
class CheckReport:
    ...
    raw_metadata: InitVar[Union[str, bytes]]

    check_metadata: CheckMetadataModel = field(init=False)

    _recommendation: LLMRecommendation = None
    _session_keeper: LLMSessionKeeper = None
    ...
    def _fill_tmpl_cmd(self, cmd: Union[str, List[str]]) -> Tuple[str]:
    ...
    async def get_next_steps(self, message: str = None):
...
async def _exec_wrapped_bash_cmd(cmd: Union[str, List]):
...
async def _exec_cmd(cmd: Union[str, Tuple[str]]):
    if not cmd:
        return
