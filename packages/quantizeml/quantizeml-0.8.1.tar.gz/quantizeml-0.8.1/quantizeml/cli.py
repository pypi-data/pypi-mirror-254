...
def quantize_model(model_path, quant_config, qparams, samples, num_samples, batch_size, epochs,
    ...
    """
    _check_model_extension(model_path, extensions=["h5", "onnx"])
...
def dump_model_config(model_path, output_name=None):
    ...
    """
    _check_model_extension(model_path, extensions=["h5"])
...
def _check_model_extension(model_path, extensions):
    _, ext = os.path.splitext(model_path)
    model_ext = ext.lower()[1:]

    if model_ext not in extensions:
        raise ValueError(
            f"Unsupported model extension: '{model_ext}'. "
            f"Expected model with extension(s): {extensions}"
        )
