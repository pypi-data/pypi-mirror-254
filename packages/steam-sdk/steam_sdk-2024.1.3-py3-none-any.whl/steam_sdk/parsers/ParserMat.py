...
def get_signals_from_mat_to_dict(full_name_file: str, list_signals, bool_transpose: bool = True):
    ...
    with h5py.File(full_name_file, 'r') as simulationSignals:
        ...
        for label_signal in list_signals:
            ...
            # TODO code the reading of structure
            data = np.array(simulationSignals.get(label_signal))
            #if simulationSignals.get(label_signal).dtype == object:
                # https://stackoverflow.com/questions/17316880/reading-v-7-3-mat-file-in-python
            dict_of_pointers ={}
            if data.dtype not in [np.float64, np.int16, np.uint16]:
                print('A struct!!')
                list_of_names = list(data.flat) #.index('B0')
                list_of_pointers = [r and simulationSignals[label_signal].get(simulationSignals[label_signal][r].name, simulationSignals[label_signal][r].name) for r in data.flat]
                for name, pointer in zip(list_of_names, list_of_pointers):
                    dict_of_pointers[name]=np.array(pointer).T
                #aaa= np.array(list_of_pointers).T
                print(dict_of_pointers)
