...
from steam_sdk.drivers.DriverAPDL import DriverAPDL


class TestDriverAPDL(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Read APDL exe path from the settings file
        with open(path_settings, 'r') as stream:
            settings_dict = yaml.safe_load(stream)
        self.APDL_path = settings_dict['ANSYS_path']
        print('ANSYS_path:        {}'.format(self.APDL_path))
    ...
    def test_DriverAPDL_initialize(self):
        '''
            This test checks that the DriverAPDL class can be run initialized.
        '''

        # arrange
        APDL_path = self.APDL_path
        input_file = 'DUMMY_INPUT_FILE.inp'

        # act
        da = DriverAPDL(ANSYS_path=APDL_path, input_file=input_file,
    ...
    def test_DriverAPDL_make_callString(self):
        '''
            This test checks that the DriverAPDL class can be run initialized.
        '''
        ...
        reserve = '1024'
        n_processors = 1
        expected_callString = f'\"{APDL_path}\" -p ansys -dir \"{directory}\" -j \"{jobname}\" -s noread ' +  f'-m {memory} -db {reserve} -t -d win32 -b -i \"{input_file}\" ' + f'-o \"{output_file}\" -smp -np {n_processors}'

        # act
        da = DriverAPDL(ANSYS_path=APDL_path, input_file=input_file, output_file=output_file, directory=directory,
    ...
    # def test_runAPDL_simplest(self):
    #     '''
    #         This test checks that APDL can be run programmatically using DriverAPDL.
    #         The path of APDL executable is set to be the one of the Gitlab runner.
