...
class ParsimEventMagnet:
    ...
    def __write_analysis_QH(self, event: DataEventMagnet, dict_QH_circuits_to_QH_strips: Dict[str, List[int]],
        ...
        if new_start_time < t_earliest_starting_time:  # if smaller then lower limit, use lower limit
            if self.verbose: print(f'Earliest QH triggers earlier then lower time limit for simulation time. Simulation start time will be set to lower limit {t_earliest_starting_time}.')
    ...
    def __read_general_parameters(self, event_info: pd.Series, new_event: DataEventMagnet, dict_attribute_to_column: Dict):
        ...
        for attr_name, list_col_names in dict_attribute_to_column.items():
            ...
            if len(used_column_names) == 1:
                ...
                raise ValueError(f"More then one column for the attribute 'GeneralParameters.{attr_name}' found: {used_column_names}.")
    ...
    def __read_powering(self, event_info: pd.Series, new_event: DataEventMagnet, dict_attribute_to_column: Dict):
        ...
        for attr_name, list_col_names in dict_attribute_to_column.items():
            ...
            if len(used_column_names) == 1:
                ...
                raise ValueError(f"More then one column for the attribute 'Powering.{attr_name}' found: {used_column_names}.")
    ...
    def __read_CLIQ(self, event_info: pd.Series, new_event: DataEventMagnet, dict_attribute_to_column: Dict):
        ...
        for attr_name, list_col_names in dict_attribute_to_column.items():
            ...
            if len(used_column_names) == 1:
                ...
                raise ValueError(f"More then one column for the attribute '{attr_name}' found: {used_column_names}.")
