...
from .attributes import afHasHead
...
class Prefs(QWidget):
    def __init__(self, parent, ui, settings):
        ...
        self._attributeFiltersDict = dict()
    ...
    def updateTabPrefs(self):
        ...
        # key=python module name
        # value={[{filter fullpath: val},{filter description: val}, [{parametro: valore}+]]

        # read the parameters of the filters stored in settings, if present
        # self._attributeFiltersDict = self._settings.readSettingAsObject('attributeFilters')
        # storedFilters = self._attributeFiltersDict['filters'].keys()

        # scans the attribute filters directory attributes/ and gets all the attribute filter classes
        foundFilters = sorted([m.__name__ for m in sys.modules.values() if m.__name__.startswith('af')])

        if len(foundFilters) > 0:
            self._ui.cbAttribSettings.addItems(foundFilters)
