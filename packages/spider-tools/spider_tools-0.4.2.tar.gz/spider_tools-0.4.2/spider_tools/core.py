...
class Auto_sinsert():
    def __init__(self, host='127.0.0.1', username='root', password='mysql', db='test',
                 drop_column=["id", "updated",'entid','created','isonline','islocal'], pool_db=False, pool_num=10,port=3306):
        self.host = host
        ...
        if pool_db:
            ...
            self.conn = pymysql.connect(host=self.host, user=self.username, passwd=self.password, db=self.db,port=port, charset='utf8mb4')
            self.cursor = self.conn.cursor()
    ...
    def execute_sql(self,sql,params=None,mode=None,keyvalue=False):
    ...
    def insert_data(self, item, table_name):
        ...
        if item and sql_conn and cursor:
            ...
            item_key_new = ['`%s`'%i for i in item_key]
            if item_key:
                item_values = [item.get(i) for i in item_key]
                sss = ''
                for i in range(len(item_key)):
                    sss += '%s,'
                insert = 'replace into {}('.format(table_name) + ','.join(item_key_new) + ')' + 'values({})'.format(
                    sss[:-1])
    ...
    def insert_data_many(self, item_list, table_name,num=500,insert_way='replace'):
        sql_conn = self.sql_pool.connection()
        cursor = sql_conn.cursor()
        AllDataList = []
        insert = None
        for item in item_list:
            if item and sql_conn and cursor:
                ...
                if item_key:
                    ...
                    for i in range(len(item_key)):
                        sss += '%s,'
                    insert = '{} into {}('.format(insert_way,table_name) + ','.join(item_key_new) + ')' + 'values({})'.format(
                        sss[:-1])
                    data = '(' + ','.join(['%r' % str(i) for i in item_values]) + ')'
                    data_list = []
                    for i in eval(data):
                ...
                if not cursor and not sql_conn:
                    ...
                    print("item is None")
        for data in list_of_groups(AllDataList,num):
            cursor.executemany(insert,data)
            sql_conn.commit()
            print("****************   {} 表  insert many {} data success   ****************".format(table_name,num))
        cursor.close()
        sql_conn.close()

class Auto_indb():
    def __init__(self,host='', username='', password='', db='',port=3306, comment='', create_tables=True):
        self.host = host
        self.username = username
        self.password = password
        self.db = db
...
class Save_csv():
    ...
    def insert_data(self, item):
        ...
        """
        if not self.csvfile:
            self.keyword_list = list(item.keys())
            self.csvfile = open(self.path, "a", newline='', encoding='utf-8')
        ...
        try:
            writer = csv.DictWriter(self.csvfile, fieldnames=self.keyword_list)
            writer.writerow(item)  # 按行写入数据
            print(f"****************   {self.path}   insert data success   ****************")
        except Exception as e:
            print(e)

class savedata():
    def __init__(self,mode='mysql_s',database_config=None,drop_column=["id", "updated", "entid", 'isonline', 'islocal']):
        self.mode = mode
        if mode=='mysql_s':
            self.auto_sinsert = Auto_sinsert(host=database_config.get('host'), username=database_config.get('user'),
                                             password=database_config.get('passwd'), db=database_config.get('db'),port=database_config['port'],
                                             drop_column=drop_column,
                                             pool_db=True)
        elif mode=='mysql_i':
            self.auto_indb = Auto_indb(host=database_config.get('host'),username=database_config.get('user'),password=database_config.get('passwd'),db=database_config.get('db'),port=database_config['port'],comment='表注释',create_tables=True)
        elif mode=='csv':
            self.save_csv = Save_csv(database_config)
        else:
            raise TypeError(f'{mode} 未支持的保存数据类型！')

    def insert_data(self,item,table_name=None):
        if self.mode=='mysql_s':
            self.auto_sinsert.insert_data(item=item,table_name=table_name)
        elif self.mode=='mysql_i':
            self.auto_indb.insert_data(items=item,table_name=table_name)
        elif self.mode=='csv':
            self.save_csv.insert_data(item=item)
        else:
            raise TypeError(f'{self.mode} 未支持的保存数据类型！')

    def insert_data_many(self,item_list,table_name=None,num=500):
        if self.mode=='mysql_s':
            self.auto_sinsert.insert_data_many(item_list=item_list,table_name=table_name,num=num)
        # elif self.mode=='mysql_i':
        #     self.auto_indb.insert_data(items=item,table_name=table_name)
        # elif self.mode=='csv':
        #     self.save_csv.insert_data(item=item)
    ...
    def execute_sql(self,sql,mode=None):
        '''
        :param sql:
        :param mode: fetch 模式取数据字段
        :return:
        '''
        if self.mode=='mysql_s':
            return self.auto_sinsert.execute_sql(sql,mode)
        elif self.mode=='mysql_i':
            return self.auto_indb.execute_sql(sql,mode)
        else:
            raise TypeError(f'{self.mode} 未支持的保存数据类型！')
...
class UPLOAD_FILE():
    def __init__(self, subfilename, key="", password="", net_address="", db=""):
        ...
        self.subfilename = subfilename + '/' + datetime.datetime.now().strftime("%Y%m/") + '{}'.format(
            datetime.datetime.now().day).zfill(2)  # oss 路径# oss 路径
        print('当前oss根路径:',self.subfilename)
...
def get_ip(proxy_url,format='json'):
    if not proxy_url:return None
    while True:
        try:
            if format=='json':
                proxy = requests.get(url=proxy_url,timeout=5,stream=False).json()
            else:
                ip = requests.get(url=proxy_url,timeout=5,).text.split('\r\n')[0].strip()
                proxy = {'http':ip,'https':ip}
            return proxy
        except Exception as e:
            time.sleep(3)
            print(f'获取代理异常！ {e.args}')
