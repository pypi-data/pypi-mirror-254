...
from airfold_cli.models import Config, NamedParam, OutputDataFormat
...
def query(
    ...
    config: Config = load_config()
    api = AirfoldApi(config.key, config.endpoint)
    jsons = api.pipe_get_data(name=name, format=format, params={p.name: p.value for p in params} if params else None)
