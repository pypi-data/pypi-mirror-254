...
class DataResourceList(BaseResourceList):
    ...
    def create_upload_params(self):
        return self.api.send(endpoint=f"{self.endpoint}upload_params", method="POST")

    def create_from_local_excel(self, local_excel_path):

        download_url = self.upload_file(local_excel_path)
