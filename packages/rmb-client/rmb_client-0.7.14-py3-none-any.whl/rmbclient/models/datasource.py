...
from rmbclient.upload import upload_to_oss, upload_to_oss
...
class DataResourceList(BaseResourceList):
    ...
    def create_from_local_excel(self, local_excel_path, direct_to_oss=True):
        if direct_to_oss:
            download_url = self.upload_file_to_oss(local_excel_path)
        else:
            download_url = self.upload_file_to_server(local_excel_path)
