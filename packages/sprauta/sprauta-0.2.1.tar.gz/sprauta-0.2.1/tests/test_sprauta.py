...
def test_visit_dependencies_cached():
    """Test that dependency functions are not evaluated multiple times."""

    from sprauta import Depends, visit

    def get_d(d: int):
        assert False  # pragma: no cover

    def get_a(d: Annotated[int, Depends(get_d)]):
        assert False  # pragma: no cover

    def get_b(d: Annotated[int, Depends(get_d)]):
        assert False  # pragma: no cover

    def f(a: Annotated[int, Depends(get_a)], b: Annotated[int, Depends(get_b)]):
        assert False  # pragma: no cover
