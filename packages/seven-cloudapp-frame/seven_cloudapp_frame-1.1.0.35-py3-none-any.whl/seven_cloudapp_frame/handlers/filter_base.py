...
@LastEditTime: 2023-12-06 16:56:47
...
def filter_check_current_limit(handler_name=None, current_limit_count=0):
    ...
    def check_current(handler):
        def wrapper(self, **args):
            ...
            if RiskManageHelper.check_current_limit(app_id, limit_count, handler_name) == True:
                self.response_json_error("current_limit", "当前人数过多,请稍后再试")
                return
            if handler_name:
                RiskManageHelper.add_current_limit_count(app_id, open_id, handler_name)
