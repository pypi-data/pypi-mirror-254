...
class IpBaseModel():
    ...
    def update_ip_info_status(self,app_id,ip_id,is_del):
        ...
        if not ip_info_dict or SafeHelper.check_app_id(ip_info_dict["app_id"], app_id) == False:
            invoke_result_data.success = False
            invoke_result_data.error_code = "error"
            invoke_result_data.error_message = "ip信息不存在"
            return invoke_result_data
    ...
    def save_ip_type(self,app_id,act_id,ip_type_id,type_name,sort_index,is_release,type_pic=""):
        ...
        if ip_type_id > 0:
            ...
            if not ip_type or SafeHelper.check_app_id(ip_type.app_id, app_id) == False:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
    ...
    def release_ip_type(self,app_id, ip_type_id, is_release):
        ...
        if not ip_type_dict or SafeHelper.check_app_id(ip_type_dict["app_id"], app_id) == False:
            invoke_result_data.success = False
            invoke_result_data.error_code = "error"
            invoke_result_data.error_message = "ip信息不存在"
            return invoke_result_data
