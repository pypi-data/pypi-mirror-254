...
def build_transform_from_cfg(
    ...
    if isinstance(cfg, list):
        transforms = [
            build_transform_from_cfg(_cfg, registry, default_args) for _cfg in cfg
        ]
        return Compose(transforms)
    else:
        if cfg is not None and (cfg.get('type', None) == 'LoadImaged' or cfg.get('type', None) == 'LoadImage'):
            if cfg.get('reader', None) == 'PILReader':
                color = cfg.pop('color', None)
                assert color in color, "color参数错误"
                cfg['converter'] = lambda x: x.convert(color)
        return build_from_cfg(cfg, registry, default_args)
