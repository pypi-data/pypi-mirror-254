...
class OpenVoidException(Exception):
    """Base Exception class, returned when nothing more specific applies"""

    def __init__(self, message: Optional[str] = None) -> None:
        super(OpenVoidException, self).__init__(message)
...
class OpenVoidAPIException(OpenVoidException):
...
class OpenVoidAPIStatusException(OpenVoidAPIException):
    """Returned when we receive a non-200 response from the API that we should retry"""

class OpenVoidConnectionException(OpenVoidException):
