...
def test_authenticate_404():
    ...
    rsp1 = responses.Response(
        method="GET",
        url="https://testpulse-io-app.fly.dev/api/tokens/check",
        status=404
    )
    responses.add(rsp1)
...
@responses.activate
@mock.patch.dict(os.environ, env_variables_mocked)
def test_authenticate_wrong_user():
    # Register via 'Response' object
    rsp1 = responses.Response(
        method="GET",
        url="https://testpulse-io-app.fly.dev/api/tokens/check",
        status=200,
        json={'id': 1234, 'name': 'wrong-user'}
    )
    responses.add(rsp1)

    with pytest.raises(HTTPRequestFailed, match='The token does not correspond to the owner.*'):
        authenticate()
