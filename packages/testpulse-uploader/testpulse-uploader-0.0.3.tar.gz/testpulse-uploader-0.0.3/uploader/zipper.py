...
def find_files(regex_pattern: Pattern[str], root: Path) -> List[str]:
    ...
    if not test_results:
        print("We did not find any file using the REGEX passed via parameters. Please check the files are there.")
        return test_results
...
def zip_files(files_list: List[str], root: Path) -> Optional[str]:
    if not files_list:
        print('Preemptive return since list of files is empty.')
        return None
