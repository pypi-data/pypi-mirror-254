...
class AltibaseDialect(default.DefaultDialect):
    ...
    def _get_default_schema_name(self, connection):
        return self.normalize_name(
            connection.exec_driver_sql(
                "SELECT DB_USERNAME FROM V$SESSION;"
            ).scalar()
        )
    ...
    def get_table_names(self, connection, schema=None, **kw):
        ...
        if self.exclude_schemaspaces:
            query += (
                    "U.USER_NAME NOT IN (%s) AND "
                    % (", ".join(["'%s'" % ts for ts in self.exclude_schemaspaces]))
            )
        query += (
            "T.TABLE_TYPE = 'T' "
            "AND T.USER_ID = U.USER_ID "
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        results = connection.execute(text(query), dict(
    ...
    def _get_constraint_data(
        ...
        query = (
            """SELECT CONST.CONSTRAINT_NAME CONSTRAINT_NAME
        ...
        rp = connection.execute(text(query), dict(table_name=self.denormalize_name(table_name), user_name=self.denormalize_name(schema)))
    ...
    def get_view_names(self, connection, schema=None, **kw):
        ...
        cursor = connection.execute(
            query, dict(user_name=self.denormalize_name(schema))
        )
        return [self.normalize_name(row[0]) for row in cursor]
