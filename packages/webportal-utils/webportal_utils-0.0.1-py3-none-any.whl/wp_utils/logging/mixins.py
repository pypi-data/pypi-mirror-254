...
class LoggingMixin:
    ...
    def __decorate(self, func: Callable, func_name: str):
        ...
        def decorator(data):
            self.app_logger.info(
                f"{self.__class__.__name__}.{func_name}() payload={data.get('body')}",
            ...
            try:
                ...
                self.app_logger.error(
                    f"{self.__class__.__name__}.{func_name}() error={str(ex)}",
                    extra=self.__build_extra(data=data, status=500),
                )
                raise
            self.app_logger.info(
                f"{self.__class__.__name__}.{func_name}() message={response.get('status', {}).get('message')}",
    ...
    def __build_extra(self, data: dict = None, response: dict = None, **kwargs):
        ...
        if data:
            extra.update(
                {
                    "request_body": json.dumps(dict(data.get("body"))),
