from .read_file import send_message_user
import pandas as pd
from dateutil.relativedelta import relativedelta
from datetime import datetime
import datareceipt as dc
class QualityCheck():
            ...
            if seperate_table[0] == 'ods':
                ...
                if i == 1:
                    if seperate_table[i] in ('amz','wb','app','sc','vc'):
                        ...
                        print(f"{i} position of the table's name is {seperate_table[i]}. The {i} position for this table should be {self.standard_name_ods[i]}. Please double check it!")
                elif i >1:
                    print(f"{i} position of the table's name is {seperate_table[i]}. The {i} position for this table should be{self.standard_name_ods[i]}. Please carefully check it!")
                ...
                if i == 1:
                    ...
                    try:
                        ...
                        print(f"{i} position of the table's name is {seperate_table[i]}. Please carefully check it!")
            else:
                    print(f"{i} position of the table's name is incorrect! Please find the suitable name for this position! The enums for {i} position are ('tb','bi','map').")
    ...
    def check_strange_data(self,checked_col,date_col,x1='',cur_start_date='',cur_end_date='',prev_start_date='',prev_end_date='',notification='disabled',user_id = ''):
        ...
        if notification != 'disabled':
            content = result
            db.send_user_message(user_id,content)  
        return result
