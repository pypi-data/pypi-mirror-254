...
def custom_job_task(
    ...
    log.info(f"Loading job specs from {temp_dir}/job_specs.pkl...")
    with fsspec.open(f"{temp_dir}/job_specs.pkl", "rb") as f:
    ...
    log.info(f"Pickling the job returns to {temp_dir}/job_returns.pkl...")
    with fsspec.open(f"{temp_dir}/job_returns.pkl", "wb") as f:
