...
class AzureCognitiveSearchStore(IndexStore):
    ...
    def delete_documents(self, documents: List[Any]):
        ...
        if len(failed) > 0:
            logger.info(f"[{self.__class__.__name__}][{self.delete_documents.__name__}] Retrying {len(failed)} documents")
    ...
    def upload_documents(self, documents: List[Any]):
        ...
        if len(failed) > 0:
            logger.info(f"[{self.__class__.__name__}][{self.upload_documents.__name__}] Retrying {len(failed)} documents")
            failed_ids = [fail["key"] for fail in failed]
    ...
    def create_upload_payload_from_embedded_document(
        ...
        to the index.
        """
        if document_data is None or (document_embeddings is None and self._include_embeddings) or document_metadata is None:
    ...
    def _process_results(self, results, start_time, operation_name):
        ...
        if len(failed) > 0:
            for r in failed:
                ...
                logger.error(
                    f"[{self.__class__.__name__}][{operation_name}] Failed document reason: {error}")
            return failed
...
class MilvusStore(IndexStore):
    ...
    def delete_documents(self, documents: List[Any]):
        ...
        try:
            ...
            logger.error(f"Failed to delete documents from Milvus index due to: {str(e)}", exc_info=True, extra={'exception': str(e)})
...
class PineconeStore(IndexStore):
    ...
    def delete_documents(self, documents: List[Any]):
        ...
        if results != {}:
            logger.error(f"Failed to delete documents from Pinecone index, full results: {json.dumps(results, indent=2)}", extra={"properties": {"results": {json.dumps(results, indent=2)}}})
            raise RuntimeError(f"{INDEX_DELETE_FAILURE_MESSAGE_PREFIX} documents, full results: {json.dumps(results, indent=2)}")
