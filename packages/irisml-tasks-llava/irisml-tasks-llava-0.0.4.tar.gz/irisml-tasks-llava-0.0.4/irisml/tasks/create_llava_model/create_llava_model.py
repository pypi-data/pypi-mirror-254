...
class Task(irisml.core.TaskBase):
    ...
    def execute(self, inputs):
        ...
        with cm as directory:
            ...
            if len(llava_config['architectures']) != 1:
                raise ValueError("This task only supports models with a single architecture")
            model_arch = llava_config['architectures'][0]
            ...
            llava_model = model_classes[model_arch].from_pretrained(directory, **kwargs)
        ...
        model = LlavaModel(llava_model, tokenizer, self.config.temperature, self.config.top_p, self.config.max_tokens)
...
class LlavaModel(torch.nn.Module):
    ...
    def __init__(self, model, tokenizer, temperature, top_p, max_new_tokens):
        super().__init__()
        self._model = model
        self._tokenizer = tokenizer
