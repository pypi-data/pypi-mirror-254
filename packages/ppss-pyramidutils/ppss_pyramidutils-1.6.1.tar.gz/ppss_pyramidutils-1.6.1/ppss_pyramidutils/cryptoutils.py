...
class CryptUtil:
    ...
    def encryptRecursive(self, value: any, preformat = None, postformat = None, skip_keys:list = [] ) -> str:
        """
        Recursively walk through value and crypt elemets separately and json dump it to string
        """        
        self._update_key()
        l.info("CryptUtil.encrypt()")
        w = jsonwalker(lambda val: self.encryptval(val,preformat,postformat), skip_keys)
