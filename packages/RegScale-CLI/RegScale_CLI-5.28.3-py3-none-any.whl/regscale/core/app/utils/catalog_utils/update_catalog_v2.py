...
def get_update_file(api: Api, catalog_number_to_update: int) -> Union[str, bytes]:
    ...
    """
    response = api.get(
        urljoin(api.config["domain"], API_CATALOGUES_ + str(catalog_number_to_update))
    )
...
def load_existing_catalog(api: Api, catalog_number_to_update: int) -> dict:
    ...
    for control in track(
        ...
        control = api.get(
            urljoin(api.config["domain"], API_SECURITY_CONTROLS_ + str(control["id"]))
        ).json()
        if (
    ...
    for control in track(
        ...
        # CCIs
        more_ccis = api.get(
            urljoin(api.config["domain"], f"api/cci/getByControl/{control['id']}")
        ).json()
            ...
            if (
                ...
                if dryrun is False:  # but ONLY if this is NOT a dry run
                    ...
                    response = api.put(
                        url=urljoin(
                            api.config["domain"],
                            API_SECURITY_CONTROLS_
                            + str(existing_map[control_id]["id"]),
                        ),
...
def do_create(
    ...
    if control_mapped is True and new_map[identifier]["archived"] is False:
        ...
        if dryrun is False:
            response = api.post(
                url=urljoin(api.config["domain"], endpoint), json=new_map[identifier]
            )
            if not response.ok:
                logger.error(
...
def check_catalog_metadata(
    ...
    if current_changes_count < len(
        ...
        if dryrun is False:  # ONLY if this is NOT a dry run
            response = api.put(
                url=urljoin(
                    api.config["domain"], API_CATALOGUES_ + str(existing_catalog["id"])
                ),
