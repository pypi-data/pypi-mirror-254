...
def parse_backmatter(resource_elem, back_matter, root, ns, ssp_id, events_list) -> dict:
    """
    A function to parse a <resource> element from an SSP and post it to RegScale
    :param resource_elem: The data element that is being passed to be parsed into a resource
    :param back_matter: The back-matter element of the SSP
    :param root: The root element of the xml document
    :param ns: Namespace dict to use for xpath element selection
    :param ssp_id: SSP ID in RegScale
    :param events_list: List of events to send to logs
    ...
    for uuid, resource in resources.items():
        ...
        if filtered_elements := [
            el for el in matching_elements if back_matter not in el.iterancestors()
        ]:
...
def reference_to_dict(reference: Element, ns) -> dict:
    """
    ...
    if (
        reference.find("ns1:title", namespaces=ns) is not None
    ):  # first check for title element
        ref["title"] = reference.find("ns1:title", namespaces=ns).text
    elif (
        reference.find("ns1:caption", namespaces=ns) is not None
    ):  # if not that, use caption for title
        ref["title"] = reference.find("ns1:caption", namespaces=ns).text
    elif (
        reference.find(DESC, namespaces=ns) is not None
    ):  # if no caption, use description up to to 50 char
...
def resource_to_dict(resource: Element) -> dict:
    ...
    for child in resource:
        tag = str(child.tag).split("}")[
            -1
        ]  # Convert tag to string and then strip namespace
...
def record_resource(resource_elem, root, ns, ssp_id, events_list, tags=None) -> None:
    """
    A function to parse a <resource> element from an SSP and post it to RegScale
    :param resource_elem: The data element that is being passed to be parsed into a resource
    :param root: The root element of the xml document
    ...
    :param ssp_id: SSP ID in RegScale
    :param events_list: List of events to send to logs
    :param tags: Optional string of semicolon-delimited tags to identify specific critical assets like network diagrams, defaults to None
    :return: None
    """
    app = Application()
    api = Api(app)
    ...
    resource_link_id = (
        resource_elem.find("ns1:link", namespaces=ns).attrib["href"].replace("#", "")
    )  # get uuid URI
    ...
    if resource.find("ns1:description", namespaces=ns) is not None:
        ref["description"] = ""
        for p in resource.find("ns1:description", namespaces=ns).findall(
            "ns1:p", namespaces=ns
        ):
            ref["description"] = ref["description"] + p.text + " "
    ...
    if (
        resource.find("ns1:title", namespaces=ns) is not None
    ):  # first check for title element
        ref["title"] = resource.find("ns1:title", namespaces=ns).text
    elif (
        resource.find("ns1:caption", namespaces=ns) is not None
    ):  # if not that, use caption for title
        ref["title"] = resource.find("ns1:caption", namespaces=ns).text
    elif (
        resource_elem.find("ns1:caption", namespaces=ns) is not None
    ):  # also check referring element for caption
        ref["title"] = resource_elem.find("ns1:caption", namespaces=ns).text
    elif (
        resource.find("ns1:description", namespaces=ns) is not None
    ):  # if no caption, use description up to to 50 char
        ref["title"] = ref["description"][:50]
    else:  # if all else fails, just make it "Untitled
        ref["title"] = "Untitled"
    ...
    for rlink in resource.findall("ns1:rlink", namespaces=ns):
        new_reference = (
            ref  # start a new instance of a reference with the shared default values
        )
    ...
    for base64elem in resource.findall("ns1:base64", namespaces=ns):
        ...
        file_metadata = {
            ...
            "filesize": len(base64elem.text) * 0.75
            - (base64elem.text[len(base64elem.text) - 2 :].count("=")),
        }
        if "media-type" in base64elem.attrib:
            file_metadata["mimeType"] = base64elem.attrib["media-type"]
        else:
            file_metadata["mimeType"], whatever = mimetypes.guess_type(
                file_metadata["trustedDisplayName"]
            )
        if tags:
...
def post_reference(reference_dict) -> None:
    """
    A function to post a reference to RegScale
    :param reference_dict: Reference object to post to RegScale
    :return: None
    """
    app = Application()
    api = Api(app)
...
def upload_base64_file(file_metadata, filestring, ssp_id):
    app = Application()
    api = Api(app)
