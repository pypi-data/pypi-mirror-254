...
class SendReminders:
    ...
    def __init__(self, app: Application, days: int = 30):
        """
        Initialize the SendReminders class
    ...
    def get_and_send_reminders(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                "Response: %i: %s=%s", res.status_code, res.reason, res.text
            )
            self.logger.debug(res.json())
            res = res.json()
        except JSONDecodeError as ex:
            error_and_exit(
                f"Unable to retrieve tenant information from RegScale.\n{ex}"
            )
        ten_id = res["tenantId"]
        ...
        with self.job_progress:
            ...
            if len(self.tenant_pipeline) > 0:
                self.logger.info(
                    "Analyzing pipeline for %s user(s).", len(self.tenant_pipeline)
                )
                # start a console progress bar and threads for the given task
                ...
                dict_users = {
                    self.activated_users[i]["id"]: self.activated_users[i]
                    for i in range(len(self.activated_users))
                }
                ...
                self.logger.info(
                    "Sending an email to %s user(s).", len(self.final_pipeline)
                )
                # start a console progress bar and threads for the given task
        ...
        self.logger.info(
            "Successfully sent an email to %s user(s)...", self.email_data.Emailed.sum()
        )
        console = Console()
        console.print(
            f"[green]Successfully sent an email to {self.email_data.Emailed.sum()} user(s)..."
        )
    ...
    def get_upcoming_or_expired_items(self, user: dict) -> Optional[Pipeline]:
        ...
        if (
            total_tasks := sum(
                len(pipeline["Pipeline"]) for pipeline in pipelines.values()
            )
            > 0
        ):
    ...
    # flake8: noqa: C901
    def analyze_pipeline(self, args: Tuple, thread: int):
        """
                    ...
                    for item in pipelines[pipe]["Pipeline"]:
                        ...
                        for key in item.keys():
                            ...
                            if key.lower() == "uuid" or (
                                ...
                                url += (
                                    f'<a href="{href}" style="mso-hide:all;">View</a>'
                                )
    ...
    def format_and_email(self, args: Tuple, thread: int):
        """
