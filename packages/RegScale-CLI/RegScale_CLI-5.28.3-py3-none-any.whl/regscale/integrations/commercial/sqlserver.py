...
@click.option(
    "--password", "-pwd", default=os.getenv("sqlServerPassword"), help="Password."
)
@regscale_id()
@regscale_module()
@click.option(
...
def generate_and_save_report(
    ...
    if not df.empty:
        ...
        description = (
            f"Sql Server Report assessment {regscale_module.title()}-{regscale_id}"
        )
...
def upload_files_to_assessment(api: Api, assessment_id: int, files: list) -> None:
    ...
    upload_files = job_progress.add_task(
        "[#0866b4]Uploading files to the new RegScale Assessment...", total=len(files)
    )
...
def create_and_save_assessment(
    ...
    """
    api = Api(app)
    new_assessment = create_assessment_object(
        app, report, regscale_id, regscale_module, title, description
    )
    create_assessment = job_progress.add_task(
        "[#21a5bb]Creating assessment in RegScale...", total=1
    )
