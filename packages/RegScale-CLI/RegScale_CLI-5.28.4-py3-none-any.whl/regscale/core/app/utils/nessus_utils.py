    ...
    if download:
        ...
        if response.status_code == 200:
            with zipfile.ZipFile(io.BytesIO(response.content), "r") as zip_ref:
                zip_ref.extractall(
                    "artifacts"
                )  # You can specify a different folder if you want
            logger.info(
                "File downloaded and extracted successfully to %s", ARTIFACTS_PATH
            )
        else:
            logger.error(
                "Failed to download the file. Status code: %i", response.status_code
            )
    return save_path


def filter_severity(
    list_of_vulns: List[Vulnerability], minimum_severity: str
) -> List[Vulnerability]:
    """
...
def get_cpe_data() -> Tuple[etree._ElementTree, List[dict]]:
    """
    Get the CPE data

    :return: CPE Root and CPE data
    :rtype: Tuple[Element, List[dict]]
    """
    cpe_list = []
...
def get_due_delta(app, severity) -> int:
    """
...
def get_minimum_severity(app) -> str:
    """
    ...
    if "minimumSeverity" in config["issues"]["tenable"]:
        minimum_severity = app.config["issues"]["tenable"]["minimumSeverity"]
    else:
        # update config
        config["issues"]["tenable"]["minimumSeverity"] = minimum_severity
        app.save_config(config)
    return minimum_severity


def lookup_kev(cve: str, data: Optional[list[dict]] = None) -> Tuple[Any, str]:
    """
...
def cpe_xml_to_dict(cpe_root: etree._ElementTree) -> List[dict]:
    ...
    :param Element cpe_root: cpe root element
    :return: cpe_items
    :rtype: List[dict]
    """
    mitre_reference_ns = (
        "{http://cpe.mitre.org/dictionary/2.0}references/"
        "{http://cpe.mitre.org/dictionary/2.0}reference"
    )
    cpe_items = []
    cpe_json = Path.cwd() / "artifacts" / "cpe_items.json"
    if not cpe_json.exists():
        logger.info("Creating cpe_items.json on initial run.")
        for cpe_item in cpe_root.iterfind(
            ...
            references = [
                ref.get("href") for ref in cpe_item.iterfind(mitre_reference_ns)
            ]
...
def lookup_cpe_item_by_name(name: str, cpe_items: List[dict]) -> Optional[dict]:
    ...
    :param list cpe_items: list of cpe-item dicts
    :return: CPE item
    :rtype: dict or None
    """
...
def software(cpe_items: List[dict], report_host: Element) -> List[dict]:
    ...
    :param Element cpe_items: the list of cpes
