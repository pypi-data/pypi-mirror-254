...
def start():
    ...
    with progress:
        ...
        directories = set_directory_variables(
            task=task0, evidence_folder=config["evidenceFolder"]
        )
        ...
        data = gather_test_project_data(
            api=api, evidence_folder=directories[0], task=task10
        )
        ...
        create_child_assessments(
            api=api, project_data=data, output=html_output, task=task16
        )
...
def package_builder(regscale_id: int, regscale_module: str, path: Path) -> None:
    ...
    with progress:
        ...
        try:
            ...
            if ssp["attachments"]:
                ...
                for f in outter_attachments:
                    ...
                    file = api.get(
                        url=config["domain"]
                        + f"/api/files/downloadFile/{file_id}/{module}/{stored_name}/{file_hash}"
                    ).content
            ...
            if ssp["normalizedControls"]:
                ...
                for f in control_attachments:
                    ...
                    file = api.get(
                        url=config["domain"]
                        + f"/api/files/downloadFile/{id}/{module}/{stored_name}/{file_hash}"
                    ).content
            ...
            progress.update(task, advance=1)
            app.logger.info(
                "An evidence zipfile has been created and is ready for use!"
            )
        except Exception:
        ...
        return app.logger.info("Evidence zipfile located. Thank you!")
...
def set_directory_variables(task, evidence_folder: str) -> Tuple[str, str, str]:
    """
...
def gather_test_project_data(api: Api, evidence_folder: str, task) -> list[dict]:
    """
...
def create_child_assessments(
    api: Api, project_data: list[dict], output: list[dict], task
) -> None:
    """
