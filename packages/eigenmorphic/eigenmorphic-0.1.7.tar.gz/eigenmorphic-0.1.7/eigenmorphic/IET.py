...
def rauzy_loop_substitution(per, loop, flips=None, max_len=1000, get_preperiod=False, verb=False):
	"""
	...
	INPUT:
		...
		- ``get_preperiod`` - boll (default: ``False``) - if True, return a couple of substitutions corresponding to (pre-period, period)
		...
		for i in range(max_len):
			...
			if tuple(v) in seen:
				if get_preperiod:
					...
					return prod(ls[:seen[tuple(v)]]), prod(ls[seen[tuple(v)]:]+[s0])
				else:
