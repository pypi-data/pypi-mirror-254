...
class WordSearch:
    ...
    def __init__(self, bank: Iterable[str], width: int, height: int, max_attempts: int = 2000) -> None:
    ...
    def _starting_positions(
        ...
        for direction, generator in STARTING_POSITION_GENERATORS.items():
            result.extend(
                (idx, direction) for idx in generator(width, height, length)
            )
        return result
    ...
    def populate_grid(self, max_attempts: int) -> None:
        ...
        for _ in range(max_attempts):
            ...
            for word in self.bank:
                starting_positions = WordSearch._starting_positions(self.width, self.height, len(word))
            ...
            if not failed:
                break
        if failed:
            raise PlacementError(f"failed to create word search in {max_attempts} attempts")
