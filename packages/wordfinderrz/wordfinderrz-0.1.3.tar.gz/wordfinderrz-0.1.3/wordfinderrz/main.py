...
class WordSearch:
    ...
    def connection_coefficient(self) -> float:
        ...
        return (
            self._connection_coefficient
            if self._connection_coefficient is not None
            else sum(len(word) for word in self.bank) / (self.width * self.height)
        )
            ...
            for word in self.bank:
                ...
                if random.random() < self.connection_coefficient:
                    with suppress(PlacementError):
                        ...
                        continue
                if place_random:
                    # Get len(starting_positions), get rand int and pop index.
                    # Keep track of decrementing len.
