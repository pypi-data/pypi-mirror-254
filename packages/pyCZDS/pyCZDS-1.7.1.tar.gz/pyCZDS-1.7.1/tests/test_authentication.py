...
class TestAuthentication(TestPyCZDS):
    ...
    def test_jwt_payload_raises_empty_token_offline(self):
        # If token is empty, method should raise a ValueError.
        self.client._token = str()
        with self.assertRaises(ValueError):
            j = self.client._get_token_jwt_payload()

    def test_not_authenticated_token_empty_offline(self):
        # Client should not be authenticated if there's no token saved.
        self.client._token = ''
        self.assertFalse(self.client._is_authenticated())

    def test_not_authenticated_token_expired_offline(self):
