...
class Bucket(Base):
    ...
    def put_bucket_acl(self, permission):
        ...
        :param str permission: 新的ACL，可以是oss.ACL_PRIVATE、oss.ACL_PUBLIC_READ或oss.ACL_PUBLIC_READ_WRITE
        """
    ...
    def list_objects(self, prefix='', delimiter='', marker='', max_keys=100, allow_unordered='false', headers=None):
        ...
        if allow_unordered == 'true':
            params={'prefix': prefix,
                   'marker': marker,
                    'max-keys': str(max_keys),
                    'allow-unordered': allow_unordered}
        else:
            params={'prefix': prefix,
                    'delimiter': delimiter,
                    'marker': marker,
                    'max-keys': str(max_keys),
                    'allow-unordered': allow_unordered}
        resp = self.__do_object('GET', '', params = params, headers=headers)
    ...
    def put_object_acl(self, key, permission):
        ...
        :param str key: 文件名
        :param str permission: 可以是oss.ACL_DEFAULT、oss.ACL_PRIVATE、oss.ACL_PUBLIC_READ或oss.ACL_PUBLIC_READ_WRITE。
    ...
    def sign_url(self, method, key, expires, headers=None, params=None, slash_safe=False):
        ...
        常见的用法是生成加签的URL以供授信用户下载，如为log.jpg生成一个5分钟后过期的下载链接::
            ...
            r'http://your-bucket.oss.cn-north-3.inspurcloudoss.com/logo.jpg?AccessKeyId=inspur-wyb2-oss&Expires=1617074629&Signature=NId6LsLcoYlquYJpkgbhgUvLjfU%3D'
