...
class dataprocessing:
    ...
    def plot_two(self, df1, col1, df2, col2, date_column, title="Plot", xlabel="Date", ylabel="Value", show_legend=True):
        """
        Plots specified columns from two different dataframes with a white background and black axes,
        using a specified date column as the X-axis. The dataframes are merged on the date column.
        ...
        :param show_legend: Boolean to show or hide legend
        ...
        try:
            ...
            raise ValueError("Error in converting date columns to datetime. Please check the format.")

        # Merging the dataframes with suffixes
        merged_df = pd.merge(df1[[date_column, col1]], df2[[date_column, col2]], on=date_column, how='outer', suffixes=('_df1', '_df2'))

        # Identify the new column names
        col1_new = f"{col1}_df1"
        col2_new = f"{col2}_df2"
    ...
    def convert_weekly_to_date(self, df, column_name, first_week_start_date):
        def get_week_start_date(week_code):
            if '-W' in week_code:
                ...
                year, week_num = map(int, week_code.split('-W'))
                year_start = datetime(year, 1, 1)
                first_monday = year_start - timedelta(days=year_start.isoweekday() - 1)
                week_start_date = first_monday + timedelta(weeks=week_num - 1)
            else:
                # Handle regular week number format
                year, week_num = map(int, week_code.split('-'))
                first_week_start = datetime.strptime(first_week_start_date, '%d/%m/%Y')
                # Adjust for the year
                first_week_start = first_week_start.replace(year=year)
