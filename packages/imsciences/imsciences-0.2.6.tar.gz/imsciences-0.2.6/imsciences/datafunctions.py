...
class dataprocessing:
    ...
    def startweek_mapping(self, df, week_col='week', start_day_of_week='Monday', date_format='%d/%m/%Y'):
        days_of_week = {'Monday': 0, 'Tuesday': 1, 'Wednesday': 2, 'Thursday': 3, 'Friday': 4, 'Saturday': 5, 'Sunday': 6}

        def week_to_startdate(week_str):
            try:
                year, week = map(int, week_str.split('-W'))
                first_day_of_year = datetime(year, 1, 1)
                day_of_week = first_day_of_year.isocalendar()[2] - 1  # Monday is 0
                start_of_iso_week = first_day_of_year - timedelta(day_of_week)
                
                # Calculate the start date of the week based on the specified start day
                day_offset = days_of_week[start_day_of_week]
                start_date_of_week = start_of_iso_week + timedelta(days=((week - 1) * 7) + day_offset)
        ...
        if week_col in df.columns:
            df['start_date'] = df[week_col].apply(week_to_startdate).dt.strftime(date_format)
        else:
            print(f"Column '{week_col}' not found in DataFrame.")
