...
class dataprocessing:
    ...
    def help(self):
        ...
        print("   - Usage: startweek_mapping(df")
        print("   - Example: startweek_mapping(df")
    ...
    def startweek_mapping(self, df):
        ...
        def week_to_startdate(week_str):
            ...
            if day_of_week <= 4:  # ISO 8601 weeks start on Monday
                start_of_week = first_day_of_year - timedelta(day_of_week - 1)
            else:
                start_of_week = first_day_of_year + timedelta(7 - day_of_week + 1)
            return start_of_week + timedelta(weeks=week-1)

        # Apply the function to each row in the 'week' column
        df['start_date'] = df['week'].apply(week_to_startdate).dt.strftime('%d/%m/%Y')
        return df
