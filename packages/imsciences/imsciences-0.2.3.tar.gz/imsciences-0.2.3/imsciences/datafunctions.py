...
class dataprocessing:
    ...
    def plot_two(self, df1, col1, df2, col2, date_column):
        ...
        # Make sure col1_new and col2_new are unique
        while col1_new in df1.columns:
            col1_new += "_1"
        while col2_new in df2.columns:
            col2_new += "_2"

        # Rename the columns in both dataframes
        df1.rename(columns={col1: col1_new}, inplace=True)
        df2.rename(columns={col2: col2_new}, inplace=True)

        # Merge the two dataframes on the date column
        merged_df = pd.merge(df1[[date_column, col1_new]], df2[[date_column, col2_new]], on=date_column, how='outer')
