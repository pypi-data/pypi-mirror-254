...
class EDRAccessor:
    ...
    def mount_storage(self, storage_account: str, container: str, mount_point: str, client_secret: str, storage_key: str) -> None:
        """Mount an Azure storage container to DBFS. Kind of a tangent to EDR access.
    ...
    def to_delta_table(self, table_name: str, container: str, storage_account: str, db: str = 'edr', mode: str = 'overwrite', overwrite_schema: bool = True) -> None:
        ...
        # spark doesn't write the indices to the file. reset_index() makes sure the index colum(s) are written to the table as well.
        spark_df = spark.createDataFrame(self._obj.reset_index())
