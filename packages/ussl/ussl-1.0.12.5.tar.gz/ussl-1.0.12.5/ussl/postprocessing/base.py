...
class BaseFunction(ABC):
    ...
    def _set_valid_input_data(self):
        ...
        if self.inputs_model is not None:
            try:
                self.input_json = self.inputs_model.loads(inputs)
            except exceptions.ValidationError as e:
                if isinstance(e.messages, dict):
                    raise BadInputError({f"Input data validation error": e.messages})
                else:
                    raise BadInputError({f"Input data validation error: {str(e)}": e.messages})
        else:
            self.input_json: dict = self._input_json.copy()
        # Валидируем секреты
        if self.secrets_model is not None:
            try:
                self.secrets = self.secrets_model.loads(secrets)['secrets']
            except exceptions.ValidationError as e:
                if isinstance(e.messages, dict):
