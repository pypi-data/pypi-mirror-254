...
def azure_setup(subscription, resource_group, region, account, iam_user):
    ...
    sp_id = strip_output(
        az_cli_wrapper(
            ...
            get_command=f"az ad sp list --display-name '{app_name}' --query '[0].id'",
        )
    )

    az_cli_wrapper(f"az role definition create --role-definition '{rg_role_def(sub_id)}'")
    az_cli_wrapper(f"az role definition create --role-definition '{log_role_def(sub_id)}'")
...
def az_cli_wrapper(
    ...
    if not interactive and p.returncode:
        if stderr and "already" in stderr:
            if get_command:
                return az_cli_wrapper(get_command)

            # TODO is this always fine to ignore because it means service principal exists?
            return ""
