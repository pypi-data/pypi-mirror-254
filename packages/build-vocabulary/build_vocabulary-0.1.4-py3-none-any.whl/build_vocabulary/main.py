...
class TextProcessor:
    @classmethod
    def run(cls, text):
        words_list = set(re.findall("([a-zA-Z]+)", text.lower()))
        words_list = cls.lemmatize_tokens(words_list)
        return words_list

    @staticmethod
    def lemmatize_tokens(words_list):
        words_list = set(item for item in words_list)
...
def process_and_save_text_segment(save_path, text, start, end, exclude_set):
    ...
    word_set = word_set.difference(exclude_set)
    flag = _process_and_save_word_set(word_set, save_path)
    if flag:
...
def main(text=None, file_path=None, directory=None, start=None, end=None, file_filter=None, record_exclude=None,
    ...
    info = args.info
    exclude = args.record_exclude or exclude
    ...
    if text:
        ...
        if integrate:
            save_path = os.path.join(save_root, default_save_name)
            pool = Pool()
            word_set = recursive_merge_or(results, pool)
            word_set.difference_update(exclude_word_set)
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            ...
            for word_set, path in zip(results, paths):
                ...
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                word_set.difference_update(exclude_word_set)
                flag = (_process_and_save_word_set(word_set, save_path) or flag)
            if flag:
                logging.info(msg.format(save_root))
    elif info:
        info_msg = pprint.pformat({
            "exclude dir": exclude_file_root,
            "exclude_words": list(exclude_word_set)
        })
