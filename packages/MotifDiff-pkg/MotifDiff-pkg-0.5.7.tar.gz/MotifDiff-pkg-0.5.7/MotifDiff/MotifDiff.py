...
import pandas as pd
from .util import vcfData, MEME_probNorm, MEME_FABIAN, TFFM, TFFM_with_Transformation, MCspline_fitting, mc_spline #
import torch
import numpy as np
...
@app.command()
def variantdiff(genome: str = typer.Option("hg38.fa", show_default=True),#str = typer.Option(..., help="fasta file for the genome"),
                ...
                normalization_file: str = typer.Option(default=None, help="file including normalization params. should be consistent with the transform option"),
                ...
                window:int = typer.Option(0, help="window size")
    ...
    e = time.time()
    print(f"real runtime for {out_file}_{scale}_{diff_score}_{mode} = ", e-s)


if __name__ == "__main__":
    #typer.run(variantdiff)
    app()