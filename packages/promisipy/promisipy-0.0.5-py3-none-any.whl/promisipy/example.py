...
def main():
    promises = [
        Promise(lambda i=i: get_rnm_info_from_id(i), mode="threading").start()
        for i in range(1, 100)
    ]
    profiles = [profile_resultion.error for profile_resultion in Promise.all(promises)]
    pprint(profiles)
