...
from errant.utils import convert_errant_to_prep_result
...
def main():
    ...
    print_results(best_dict, best_cats, args, 'errant.csv')
    best_dict_prep, best_cats_prep = convert_errant_to_prep_result(
        best_dict, best_cats,
    )

    logger.info('Systematic Evaluate based on ACA tags')
    print_results(best_dict_prep, best_cats_prep, args, 'prep.csv')
    ...
    for hyp_id in hyp_dict.keys():
        for ref_id in ref_dict.keys():
            ...
            tp, fp, fn, cat_dict = compareEdits(
                hyp_dict[hyp_id], ref_dict[ref_id],
            )
...
def print_results(best, best_cats, args, name_file):
    ...
    if args.cat:
        ...
        for cat, cnts in sorted(best_cats.items()):
            cat_p, cat_r, cat_f = computeFScore(
                cnts[0], cnts[1], cnts[2], args.beta,
            )
            print(
                cat.ljust(14),
                str(cnts[0]).ljust(8),
        ...
        result['Marco Avg'] = [
            None, None, None, round(
                mic_p, 4,
            ), round(mic_r, 4), round(mic_f, 4),
        ]

        mac_p, mac_r, mac_f = computeFScore(
            best['tp'], best['fp'], best['fn'], args.beta,
        )
        result['Micro Avg'] = [
            best['tp'], best['fp'],
            best['fn'], mac_p, mac_r, mac_f,
        ]
    ...
    if args.cat:
        dataframe = pd.DataFrame.from_dict(
            result,
            orient='index',
            columns=['TP', 'FP', 'FN', 'Prec', 'Rec', 'F0.5'],
        )
        dataframe.to_csv(name_file)
