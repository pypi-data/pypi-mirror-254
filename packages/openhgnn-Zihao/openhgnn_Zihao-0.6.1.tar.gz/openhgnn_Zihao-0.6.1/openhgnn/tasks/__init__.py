...
def build_task(args):  # args  就是最开始的 config
    if not try_import_task(args.task):
        exit(1)
    return TASK_REGISTRY[args.task](args)  # args.task就是注册的task名，
#                                  args 就是最外层的config
...
SUPPORTED_TASKS = { # 这里面的task尽量不要加新东西，可以写一个新的task类放到某个已有的task.py文件中
