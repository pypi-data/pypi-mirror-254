...
####################################################################
#          修改处
#      specific_trainerflow[model,task]锁定一个trainer字符串，作为build_flow时的参数flow_name
        'HAN': {
        ...
        'DHNE': 'DHNE_trainer',
###################################################
#     也可以只需要 specific_trainerflow[model] 锁定一个trainer字符串。  
...
############################################################################
      #  给self.config(也就是trainer,task,dataset中的args)添加额外参数：model,dataset,task,gpu
        #                                                           logger,seed  等
...
######################################################################
#  从specific_trainerflow这个字典中，用model和task锁定一个trainer名字符串，这就是trainerflow            
            
        #   self.config就是之后flow中的args
            flow = build_flow(args = self.config, flow_name = trainerflow)
            result = flow.train()
