...
from . import AcademicDataset, HGBDataset, OHGBDataset,IMDB4MAGNN_Dataset   # 修改处
...
class NodeClassificationDataset(BaseDataset):
    ...
    这个基类，用于节点分类任务的数据集。它的子类应该包含属性（attr）：graph,category,num_classes等等
    基类要实现函数 get_labels()和get_split()

    实际上这个节点分类数据集，这个基类没啥用，定义了两个函数，其他的都没啥用
    ...
    # 参与分类的节点类型，返回该类型中所有节点，哪些属于train,val,mask
    def get_split(self, validation=True):
        r"""
        
        一般多个类别的节点中，只有一个节点类型有labels和mask，这个函数会返回该类型节点的train_mask,val_mask
        test_mask
        ...
        # self.category，只有这一个类别有labels和mask
        ...
        # 返回的是参与分类的节点，他们的三个mask的节点id，维度都是该类型节点总数*1
...
# NodeClassificationDataset  和 BaseDataset 都只定义了一些None的成员，没什么具体操作。
...
#       父类初始化函数中，定义了一堆None，没啥具体内容
    ...
    def load_HIN(self, name_dataset):
        if name_dataset == 'demo_graph':
            ...
            # dataset = DBLP4MAGNN_Dataset(name='dblp4MAGNN')
            category = 'A'
            g = dataset[0].long()
            num_classes = 4
...
######################              修改处  #############################################
            ...
            category = 'M'  # 该类型节点参与节点分类
            g = dataset[0].long()
            num_classes = 3 # M类型节点，有3中不同的label
            self.in_dim = g.ndata['h'][category].shape[1] # 参与分类的节点，它的特征长度
            ...
            category = 'paper'
            g = dataset[0].long()  # dataset[0] == dataset.getitem(0)
            num_classes = 3
