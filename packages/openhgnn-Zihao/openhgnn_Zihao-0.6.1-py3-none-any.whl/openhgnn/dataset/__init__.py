...
from .academic_graph import AcademicDataset  , IMDB4MAGNN_Dataset  #  修改处
...
def build_dataset(dataset, task, *args, **kwargs): # 关键字参数里面可能有args，就是最外层的config
...
##################################################
    elif dataset in ['GGNN_WN18RR','GGNN_FB15k-237']:
        _dataset = 'GGNN_' + task  #  == 'GGNN_link_prediction'  这就是注册的数据集名称（模型 + 任务  等于注册的数据集名称）
                                                                        #   args就是最外层config
