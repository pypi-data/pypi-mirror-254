from _typeshed import Incomplete
from proalgotrader_core.helpers.get_totp import get_totp as get_totp
from proalgotrader_core.token_managers.base_token_manager import BaseTokenManager as BaseTokenManager
from typing import Any

AUTH_URL: str
API_URL: str
URL_SEND_LOGIN_OTP_V2: Incomplete
URL_VERIFY_TOTP: Incomplete
URL_VERIFY_PIN_V2: Incomplete
URL_TOKEN: Incomplete
URL_VALIDATE_AUTH_CODE: Incomplete
session: Incomplete

class FyersTokenManager(BaseTokenManager):
    ws_client: Incomplete
    http_client: Incomplete
    ws_access_token: Incomplete
    http_access_token: Incomplete
    username: Incomplete
    totp_key: Incomplete
    pin: Incomplete
    client_id: Incomplete
    secret_key: Incomplete
    redirect_url: Incomplete
    app_id: Incomplete
    app_type: Incomplete
    def __init__(self, username: str, totp_key: str, pin: int, client_id: str, secret_key: str, redirect_url: str) -> None: ...
    def set_token(self, token: str) -> None: ...
    def get_token(self) -> str: ...
    def login_otp(self) -> Any: ...
    def verify_otp(self, request_key: str) -> Any: ...
    def verify_pin(self, request_key: str) -> Any: ...
    def get_auth_code(self, access_token: str) -> str: ...
