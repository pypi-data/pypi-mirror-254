...
class ChannelBuilder:
    ...
    def add_interceptor(self, interceptor: Union[grpc.UnaryStreamClientInterceptor]) -> None:
    ...
    def toChannel(self) -> grpc.Channel:
        ...
        if not use_secure:
            channel = grpc.insecure_channel(destination, options=self._channel_options)
        else:
        ...
        # Apply all interceptors on the channel.
        if len(self._interceptors) > 0:
            print("Applying interceptors")
            result = grpc.intercept_channel(channel, self._interceptors[0])
            return result
        else:
            return channel
