...
_ENABLE_HTML_REPR_ENV_VAR_NAME = "_ATOTI_ENABLE_DATABASE_SCHEMA_HTML_REPR"

_MERMAID_MIME_TYPE = "text/vnd.mermaid"
"""See https://github.com/mermaid-js/mermaid/blob/6e6455632668f5f674ea43bd49ac1a653d232f0c/docs/ecosystem/integrations-create.md?plain=1#L19."""
...
def _generate_entity_relationship_diagram(schema: DatabaseSchema, /) -> str:
    indent = "  "
...
class TableDescription:
    columns: Annotated[Sequence[ColumnDescription], Field(min_length=1)]
    keys: Collection[str]
    name: str
...
class DatabaseSchema:
    ...
    tables: Collection[TableDescription]

    @override
    def __repr__(self) -> str:
        return _generate_entity_relationship_diagram(self)
    ...
    def _repr_mimebundle_(
        ...
        bundle = {_MERMAID_MIME_TYPE: diagram}

        if get_env_flag(_ENABLE_HTML_REPR_ENV_VAR_NAME):
            bundle[_HTML_MIME_TYPE] = f"""<div class="mermaid">{diagram}</div>"""
        else:
            bundle[_MARKDOWN_MIME_TYPE] = f"```mermaid\n{diagram}```\n"
