...
def get_non_personal_type_name(value: object, /) -> str:
    ...
    return (
        f"{f'{module.__name__}.{value_type.__name__}' if is_value else value_type}"  # type: ignore[attr-defined]
        if module and _is_non_personal_module(module)
        else f"{value_type.__name__ if value_type in _BUILTIN_TYPES else _PERSONAL_TYPE_NAME}"  # type: ignore[attr-defined]
    )
