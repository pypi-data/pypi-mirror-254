...
impl PyTxn {
    ...
    #[staticmethod]
    fn build(when: Option<Vec<PyCompare>>, and_then: Option<Vec<PyTxnOp>>,) -> Self {
        let when = when
        .unwrap_or_default()
        .into_iter()
        .map(|c| c.0)
        ...
        PyTxn(Txn::build(when, and_then))
    }

    // fn build(
    //     &self,
    //     compares: Option<Vec<PyCompare>>,
    //     on_success: Option<Vec<PyTxnOp>>,
    //     on_failure: Option<Vec<PyTxnOp>>,
    // ) -> PyResult<Self> {
