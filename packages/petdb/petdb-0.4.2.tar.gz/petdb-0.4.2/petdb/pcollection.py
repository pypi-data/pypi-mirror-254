...
class PetArray[T]:
	...
	def filter(self, query: dict | Callable[[T], bool]) -> Self:
	...
	def flat(self, query: str | int | Callable[[T], bool]) -> Self:
	...
	def groupby(self, key: str | int | Iterable[str | int] | Callable[[T], Any] = None) -> Self:
		"""
		...
		if isinstance(key, (str, int, NoneType)):
			...
			raise QueryException("Invalid group by key: it should be a string, integer or iterable")
		result = {}
	...
	def reduce[TP](self, reducer: Callable[[TP, T], TP], init: TP = None):
		"""

		:param reducer: function to reduce elements, takes ``accumulator`` and ``currentElement``
