...
from zippy_form.utils import FORM_STATUS, FORM_FIELD_STATUS, FIELD_TYPES, FORM_SUBMISSION_STATUS, \
    ...
    DEFAULT_STRIPE_APPLICATION_FEE_AMOUNT, PAYMENT_TYPE_DETAILS, PAYMENT_MODE_DETAILS, CURRENCY_DETAILS, convert_utc_to_timezone,get_stripe_secret_key,get_stripe_public_key,get_stripe_connect_url, \
...
def form_submission_list(req, form_id):
    ...
    if start_date and end_date:
        start_date_time_delta = timedelta(hours=0, minutes=00, seconds=00)
        end_date_time_delta = timedelta(hours=23, minutes=59, seconds=59)
        start_date = datetime.strptime(start_date, '%d/%m/%y') + start_date_time_delta
        end_date = datetime.strptime(end_date, '%d/%m/%y') + end_date_time_delta
        form_submission_filter['modified_date__range'] = [start_date,end_date]
