...
class ClickhousePool(BaseConnectionPool):
    ...
    def sync_create_pool(self):
        ...
        pool = pooled_db.PooledDB(creator=clickhouse_driver.connect, host=self.conn_info.host, port=self.conn_info.port,
                                  ...
                                  **params)
        # pool = clickhouse_driver.connect(host=self.conn_info.host, port=self.conn_info.port,
        #                                  user=self.conn_info.user,
        #                                  password=self.conn_info.password,
        #                                  database=self.conn_info.db_name or "default",
        #                                  **params)

        # pool = clickhouse_driver.Client(host=self.conn_info.host, port=self.conn_info.port,
        #                                 user=self.conn_info.user,
        #                                 password=self.conn_info.password,
        #                                 send_receive_timeout=send_receive_timeout, **params)
        return pool
