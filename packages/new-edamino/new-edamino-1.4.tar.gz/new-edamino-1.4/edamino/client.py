...
class Client:
    ...
    async def request(self,
        ...
        async with self.session.request(method=method, url=url, headers=headers, data=data, proxy=self.proxy) as resp:
            ...
            if resp.status != 200:
                print("error aminoapps: ", resp.status, " url: ", url)
                try:
    ...
    async def get_wiki_information(self, wiki_id: str) -> Dict:
        response = await self.request('GET', f'item/{wiki_id}')
    ...
    async def get_leaderboard_info(self, rankingType=2):
        response = await self.request("GET", f"https://service.aminoapps.com/api/v1/g/s-{self.ndc_id}/community/leaderboard?rankingType={rankingType}&start=0&size=100", full_url=True)
    ...
    async def get_wall_comments(self, comId: str, userId: str, sorting: str, start: int = 0, size: int = 25):
        ...
        data = {"timestamp": int(time() * 1000)}
        response = await self.request('GET', f"https://service.aminoapps.com/api/v1/x{comId}/s/user-profile/{userId}/comment?sort={sorting}&start={start}&size={size}", json=data, full_url=True)
        return response['commentList']
    
    async def moderation_history(self, userId: str, size: int = 25):
        url = f"admin/operation?objectId={userId}&objectType=0&pagingType=t&size={size}"
        response = await self.request('GET', url)
        return response['adminLogList']
    
    async def create_notification(self, comId: str, url: str, message: str, scheduled_time: int = 0):
        data = {"timestamp": int(time() * 1000), "payload": {"aps": { "alert": message }, "u": url}, "scheduledTime": scheduled_time}
        response = await self.request('POST', f"https://service.aminoapps.com/api/v1/x{comId}/s/push", json=data, full_url=True)
        return response
    
    async def get_notification(self, comId: str, scheduling: bool = False):
        url = f"https://service.aminoapps.com/api/v1/x{comId}/s/push?status={'1%2C4%2C2' if scheduling is True else '3%2C5%2C5%2C8%2C9'}"
        response = await self.request('GET', url, full_url=True)
        return response
    
    async def get_topics(self, comId: str):
        url = f"https://service.aminoapps.com/api/v1/x{comId}/s/community/main-topics"
        response = await self.request('GET', url, full_url=True)
        return response