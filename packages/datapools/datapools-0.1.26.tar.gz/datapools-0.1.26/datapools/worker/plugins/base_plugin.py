...
from ..worker import WorkerContext, WorkerTask
...
class BasePlugin(metaclass=ABCMeta):
    ...
    async def download_content(self, url, type: DatapoolContentType):
        ...
        if not await self.is_content_processed(url, storage_id):
    ...
    async def is_content_processed(self, url, storage_id):
        if await self.ctx.storage.has(storage_id):
            logger.info(f'already in storage {storage_id=}')
            return True
