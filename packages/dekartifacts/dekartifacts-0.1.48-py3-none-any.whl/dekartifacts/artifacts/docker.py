...
class DockerArtifact(ArtifactBase):
    ...
    @classmethod
    def imports(cls, path):
        if os.path.isdir(path):
            for file in os.listdir(path):
                shell_wrapper(f"{cls.cli} load -i {os.path.join(path, file)}")
        else:
            shell_wrapper(f"{cls.cli} load -i {path}")
    ...
    def build_fast(cls, path, step, images, args=None, push=True):
        ...
        def _do_build_result():
            ...
            for image_name in images:
                ...
                write_file(os.path.join(path, '.context'), ci=os.path.join(path, image_name, '.context'))
