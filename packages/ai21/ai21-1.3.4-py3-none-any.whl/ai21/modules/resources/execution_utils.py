...
from ai21.modules.destination import SageMakerDestination, BedrockDestination
...
def execute_sm_request(destination: SageMakerDestination, params, mandatory_fields=None, unsupported_fields=None):
    ...
    from ai21.aws.sagemaker import invoke_sm_endpoint  # import here because boto3 exists only in AWS mode
...
def execute_bedrock_request(destination: BedrockDestination, params = None,
    ...
    from ai21.aws.bedrock import invoke_bedrock_model  # import here because boto3 exists only in AWS mode

    return invoke_bedrock_model(
        model_id=destination.model_id,
        input_json=input_json,
        bedrock_session=destination.boto_session,
    )
