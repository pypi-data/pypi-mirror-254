...
def adjusted_win_rating(
    win_rating: FormRating,
    lengths_ahead: Horselength,
    lbs_per_length: float = 4,
    comment: str = "",
) -> FormRating:
        ...
        np.array(
            [
                ...
                for x in [
                    ...
                    (
                        1.5
                        if any(word in bonus_words for word in comment.split())
                        else 1
                    ),
                ]
            ]
    ...
    return win_rating.adjust(adjustment)
...
class FormPredictor:
    ...
    def _additions(self) -> float:
        ...
        return float(sum(b(self.horse, self.race_conditions) for b in bonuses))
    ...
    def _rated_performances(self) -> np.ndarray[float, np.dtype[Any]]:
        ...
        Returns:
            A numpy array containing past performances that have been rated.
        """
        return np.array([p for p in self.horse.performances if p and p.form_rating])
    ...
    def _weightings(self) -> np.ndarray[float, np.dtype[Any]]:
        ...
        return np.array(
            [
                float(self.race_conditions.like(key))
                * float(value.form_rating.solidity)
                for key, value in self.horse.race_career.items()
                if value and value.form_rating
            ]
        )
    ...
    def predict(self) -> FormRating | None:
        ...
        if not len(self._ratings):
            return FormRating(
                FlatRaceLevelParRating.average(self.race_conditions.race_level)
            )
        ...
        average = np.average(
            [
                float(
                    adjusted_win_rating(
                        p.form_rating, abs(min(p.beaten_distance or 0, 0))
                    )
                )
                for p in self._rated_performances
            ],
    ...
    def lower_bound(self):
        ...
        """
        race_min_val = FlatRaceLevelParRating.min(self.race_conditions.race_level)
    ...
    def upper_bound(self):
        ...
        """
        race_max_val = FlatRaceLevelParRating.max(self.race_conditions.race_level)
    ...
    def to_debug(self):
        ...
        return {
            ...
            "performance_summary": [
                {
                    "course": k.racecourse.name,
                    "date": k.datetime.datetime.format("YYMMDD"),
                    "rating": v.form_rating,
                    "likeness": float(self.race_conditions.like(k)),
                    "solidity": float(v.form_rating.solidity),
                }
                for k, v in self.horse.race_career.items()
                if v and v.form_rating
            ],
        }
