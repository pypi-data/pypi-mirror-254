...
from ditts.cli import tts_cli

# Need to look into diTTS' log output to test proper instantiation
# - Use testfixtures.LogCapture() b/c TestCase.assertLogs() needs py3.4+
# - Clear 'diTTS' logger handlers (set in diTTS.cli) to reduce test noise
import logging
from testfixtures import LogCapture

logger = logging.getLogger("diTTS")
logger.handlers = []
...
def test_lang_nocheck():
    """Invalid <lang> (with <nocheck>) should display an error message from diTTS"""
...
@pytest.mark.net
def test_params_set():
    """Options should set diTTS instance arguments (read from debug log)"""
    with LogCapture() as lc:
        result = runner_debug(
