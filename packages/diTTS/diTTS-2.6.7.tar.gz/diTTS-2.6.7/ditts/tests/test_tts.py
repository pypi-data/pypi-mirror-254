...
from ditts.tts import diTTS, diTTSError
from ditts.langs import _main_langs
from ditts.lang import _extra_langs
...
def test_TTS(tmp_path, lang):
    ...
    for slow in (False, True):
        ...
        # Create diTTS and save
        tts = diTTS(text=text, lang=lang, slow=slow, lang_check=False)
        tts.save(filename)
...
def test_unsupported_language_check():
    ...
    with pytest.raises(ValueError):
        diTTS(text=text, lang=lang, lang_check=check)
...
def test_get_bodies():
    ...
    tts = diTTS(text="test", tld="com", lang="en")
...
def test_msg():
    """Test diTTSError internal exception handling
    Set exception message successfully"""
    error1 = diTTSError("test")
...
def test_infer_msg():
    ...
    errorTLD = diTTSError(tts=ttsTLD)
    assert (
        errorTLD.msg
    ...
    error403 = diTTSError(tts=tts403, response=response403)
    assert (
        error403.msg
    ...
    error200 = diTTSError(tts=tts200, response=response404)
    assert (
        error200.msg
    ...
    error500 = diTTSError(tts=tts500, response=response500)
    assert (
        error500.msg
    ...
    error100 = diTTSError(tts=tts100, response=response100)
...
def test_WebRequest(tmp_path):
    ...
    """Raise diTTSError on unsupported language (without language check)"""
    lang = "xx"
    check = False

    with pytest.raises(diTTSError):
        filename = tmp_path / "xx.txt"
        # Create diTTS
        tts = diTTS(text=text, lang=lang, lang_check=check)
        tts.save(filename)
