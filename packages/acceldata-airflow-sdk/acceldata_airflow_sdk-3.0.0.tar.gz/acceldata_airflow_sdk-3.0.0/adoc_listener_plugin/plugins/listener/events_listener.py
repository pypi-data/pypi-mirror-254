...
def on_dag_run_failed(dag_run: DagRun, msg: str):
    ...
    if validate_and_auto_instrument_dag(dag_id):
        log.info("Observing the DAG with dag_id  %s ", dag_id)
        end_pipeline_for_dag_run(dag_run, get_torch_client(), PipelineRunResult.FAILURE, PipelineRunStatus.FAILED, msg)
    else:
        log.info("Instrumentation not enabled for the dag with dag_id %s ", dag_id)
...
def on_dag_run_success(dag_run: DagRun, msg: str):
    ...
    # If environment variables to observe and ignore both are present, log error
    if observe_dag_env_variables_detected() and ignore_dag_env_variables_detected():
        log.error(INVALID_ENV_SELECTION_MESSAGE)
        return
    ...
    if validate_and_auto_instrument_dag(dag_id):
        log.info("Observing the DAG with dag_id  %s ", dag_id)
        end_pipeline_for_dag_run(dag_run, get_torch_client(), PipelineRunResult.SUCCESS, PipelineRunStatus.COMPLETED,
                                 msg)
    else:
        log.info("Instrumentation not enabled for the dag with dag_id %s ", dag_id)
...
def on_task_instance_success(previous_state: TaskInstanceState, task_instance: TaskInstance, session):
    ...
    if validate_and_auto_instrument_dag(dag_id):
        ...
        end_pipeline_for_older_versions(task_instance, PipelineRunResult.SUCCESS, PipelineRunStatus.COMPLETED, session)
    else:
        log.info("Instrumentation not enabled for the dag with dag_id %s ", dag_id)
