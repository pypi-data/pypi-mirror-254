...
class TrainWrapper:
    ...
    def train_function(
        ...
        open("train1-1.txt", "w").write(f"train {project_name}\n")
        try:
            _train(
                project_name,
                config_path,
            ...
            open("train1-1-1eeeee.txt", "w").write(f"train {e}\n")

        open("train1-2.txt", "w").write(f"train {project_name}\n")
        self.update_status("DONE")
        open("train1-3.txt", "w").write(f"train {project_name}\n")

    def train_callback(self, data_type: str, n: int, total: int, status: str):
        open("train1-1-1.txt", "w").write("train")
        ...
        open("train1-1-2.txt", "w").write("train")
...
def train(
    ...
    open("train1.txt", "w").write(f"train {project_name}\n")
    ...
    # UUIDを作成
    open("train2.txt", "w").write(f"train {project_name}\n")
    uuid_str = str(uuid.uuid4())

    open("train3.txt", "w").write(f"train {project_name}\n")
    ...
    open("train4.txt", "w").write(f"train {project_name}\n")
    ...
    open("train5.txt", "w").write(f"train {project_name}\n")
    try:
        data = ProgressFunctionCallResponse(
            status="OK",
            uuid=uuid_str,
