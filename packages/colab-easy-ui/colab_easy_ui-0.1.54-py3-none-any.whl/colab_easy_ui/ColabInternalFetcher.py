...
class ColabInternalFetcher:
    ...
    def start_tensorboard_colab(self, tb_logs_dir):
        open("test1-1.txt", "w").write("test")
        self.tb_port = portpicker.pick_unused_port()
        if self.logfile is not None:
            open("test1-2.txt", "w").write("test")
            self.ipython.system_raw(f"tensorboard --port {self.tb_port} --logdir {tb_logs_dir} >{self.logfile} 2>&1 &")
        else:
            open("test1-3.txt", "w").write("test")
            try:
                self.ipython.system_raw(f"tensorboard --port {self.tb_port} --logdir {tb_logs_dir} >TB_LOG 2>&1 &")
            except Exception as e:
                open("test1-3e.txt", "w").write(f"test {e}")
            open("test1-4.txt", "w").write("test")
        return self.tb_port
    ...
    def start_tensorboard(self, project_name: str):
        ...
        if is_running_on_colab() is True:
            open("test.txt1", "w").write("test")
            tb_port = self.start_tensorboard_colab(tb_logs_dir)
            open("test.txt2", "w").write("test")
        else:
            open("test.txt_a", "w").write("test")
