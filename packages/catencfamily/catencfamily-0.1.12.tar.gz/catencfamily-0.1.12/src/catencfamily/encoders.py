...
class CatEncodersFamily(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,  pathToStoreProgress = None, modelsPath= None,
        ...
        verbose : int; Intensity of msgs to be displayed. Presently, this parameter takes 
                  only two values: 0 or > 0. If, > 0, all comments are displayed., If 0,
                  only one line progress is given. The default is 3.
    ...
    def fit(self, X, cat_cols , interactingCatCols = None, y=None):
        ...
        for i in range(self.n_iter):
            ...
            for k,j in enumerate(fulllist_):
                if (self.verbose) : print("Record set no is {}  ".format(k+1))
                if (self.verbose > 2) : print("Set of records is: ", j)
    ...
    def _getStoreFeatures(self,train, record_sets):
        ...
        with open(q, 'w') as myfile:
            ...
            for j,i in enumerate(record_sets):
                ...
                if (self.verbose) : print("Done with", colToProject," and ", intermediaryCol)
                myfile.write( str(1) + "," +  str(time_taken) + '\n')  # Can be used in an Excel file
                myfile.flush()
                if (self.verbose) : print("Time taken: ", time_taken)
                if (self.verbose) : print("Entry made in file progress.csv")
                gc.collect()
        return        
    ...
    def _featureEngTrainTest(self,train,colToProject, intermediaryCol):
        ...
        if (self.verbose) : print("No of unique values in ", colToProject, " are: ", noOfUniqueValues)
    ...
    def _learn(self,df,colToProject, intermediaryCol):
        ...
        end1 = time.time()
        tt = (end1-start)/60
        if (self.verbose) : print("Bipartite Network created. Time taken: ", tt, "minutes")
        ...
        end2 = time.time()
        tt1 = (end2-end1)/60
        if (self.verbose) : print("Bipartite Network projected. Time taken: ", tt1, "minutes" )
        gc.collect()
        ...
        end3 = time.time()
        tt2 = (end3-end2)/60
        if(self.verbose) : print("Model extracted. Time taken: ", tt2, "minutes" )
    ...
    def _calCentralityMeasures(self,model, network):
        ...
        if (self.cMeasures[5] == 1):
            ...
            if self.k is None:
                m = noOfnodes
                if (self.verbose): print("Calculating betweenness centrality with k = All nodes")
            else:
                ...
                if (self.verbose) : print(f"Calculating betweenness centrality with k = {m} nodes sample")
                
            start = time.time()
            bt = nx.betweenness_centrality(network, k = m)
            end = time.time()
            tt = (end-start)/60
            if (self.verbose) : print(f"Betweenness centrality calculated. Time taken: {tt} min ")
