...
def manage_synthesize(path_QASM, path_qchip, **kwargs):
	...
	if any(qchip_data.get(item) is None for item in ["cnot_gate_time", "cnot_error_rate"]):
		return False, "We need the calibration data such as cnot gate time, fidelity. "
	...
	best_last_mapping  = {k: v for k, v in best_last_mapping.items()}
