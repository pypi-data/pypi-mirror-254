...
from oaemlib.edge_provider import EdgeProvider
...
def compute_oaem(
    ...
    edge_list = edge_provider.get_edges(position)
    return oaem_from_edge_list(edge_list=edge_list, position=position, res=res)


def oaem_from_edge_list(edge_list: list[Edge], position: PointSet, res: float) -> Oaem:
    """
    ...
    return Oaem(
        position=position, azimuth=oaem_temp[:, 0], elevation=oaem_temp[:, 1], res=res
    )
