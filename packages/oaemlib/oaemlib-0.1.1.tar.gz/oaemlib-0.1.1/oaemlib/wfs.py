...
from oaemlib.gml import extract_lod1_coords, extract_lod2_coords
...
def edge_list_from_wfs(position: PointSet, wfs_settings: WFSSettings) -> list[Edge]:
    ...
    gml_reader = extract_lod1_coords if wfs_settings.lod == 1 else extract_lod2_coords

    return parse_response(response, gml_reader)
...
def parse_response(
    response: requests.Response, gml_reader: Callable[[str], np.ndarray]
) -> list[Edge]:
    """
    ...
    building_coordinates: np.ndarray = np.array(
        gml_reader(str(response.content, encoding="utf-8"))
    )
