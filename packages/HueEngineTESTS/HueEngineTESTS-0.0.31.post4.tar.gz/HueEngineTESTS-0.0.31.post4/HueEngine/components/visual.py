...
class TextureComponent:
    def __init__(self, color:_ARRAY[int]=[53,20,62], size:list|tuple=(32,32)) -> _NULL:
        self.size = size
        self.color = color
        self.texture = pg.sprite.Sprite()
        self.texture.image = pg.Surface(self.size)
    ...
    def Load(self, fp:str):
        try:
            ...
            self._ogriginalImage = fp
            image = HueEngine.scaleSurface(image, self.size)
