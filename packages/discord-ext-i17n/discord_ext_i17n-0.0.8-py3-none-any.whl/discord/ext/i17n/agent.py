...
from discord.ext.i17n.preprocess import (
    TranslationAgent,
    Translator,
)
...
def wrap_i17n_editer(edit_func: InterfaceOverridable):
    """
    ...
    async def wrapped_i17n_edit(self: Union[Message, InteractionResponse], **kwds):
        dest_lang = await AutoI17nAgent.detector.first_language_or(self)
        if dest_lang:
            kwds["content"] = TranslationAgent.encode_lang_str(
                AutoI17nAgent.source_lang, kwds.get("content", None) or "", dest_lang
            )
        return await edit_func(self, **kwds)
    
    return wrapped_i17n_edit


def i17n_HTTPClient_edit_message(
    ...
    if fields["content"]:
        ...
        if lang:
            fields, fields["content"] = TranslationAgent.translate_payload(
                lang, fields, content, **AutoI17nAgent.get_config()
            )
    return HTTPClient_edit_message(self, channel_id, message_id, **fields)

def wrap_i17n_sender(send_func: InterfaceOverridable):
    """
    ...
    if content:
        ...
        if lang:
            kwds, content = TranslationAgent.translate_payload(
                lang, kwds, payload, **AutoI17nAgent.get_config()
            )
    return HTTPClient_send_message(self, channel_id, content, **kwds)


def i17n_Adapter_create_interaction_response(self: AsyncWebhookAdapter, *args, **kwds):
    """
    ...
    if kwds["data"] and ("content" in kwds["data"] or "title" in kwds["data"]):
        ...
        if lang:
            (
                ...
                lang, kwds["data"], content, **AutoI17nAgent.get_config()
            )
    return AsyncWebhookAdapter_create_interaction_response(self, *args, **kwds)


def i17n_AsyncWebhookAdapter_execute_webhook(self: AsyncWebhookAdapter, *args, **kwds):
        ...
        if "content" in kwds["payload"] and kwds["payload"]["content"].strip():
            ...
            if lang:
                (
                    ...
                    lang, kwds["payload"], content, **AutoI17nAgent.get_config()
                )
    return AsyncWebhookAdapter_execute_webhook(self, *args, **kwds)


async def i17n_InteractionResponse_send_modal(self: InteractionResponse, modal: Modal):
    """
    ...
    """
    dest_lang = await AutoI17nAgent.detector.first_language_of(self)
    if dest_lang:
        modal.title = TranslationAgent.encode_lang_str(
            AutoI17nAgent.source_lang, modal.title, dest_lang
        )
...
class AgentSession:
    def __init__(
        ...
        ```py
        DEFAULT = AutoI17nAgent.translate_embeds
        AutoI17nAgent.translate_embeds = False
        await ctx.reply(
            embed=Embed(
                title="Theoretical Physics",
                description="This description will never be translated.",
        ...
        AutoI17nAgent.translate_embeds = DEFAULT
        ```
        """
        ...
        self.initial_config = {key: getattr(AutoI17nAgent, key, None) for key in vmap}

        for key, val in vmap.items():
            if translate_all and key.startswith("translate_"):
                setattr(AutoI17nAgent, key, True)
            elif val is not None:
                setattr(AutoI17nAgent, key, val)
...
class AutoI17nAgent:
    ...
    def __init__(
        ...
        """
        if AutoI17nAgent._instantiated:
            raise TypeError("this class should only be instantiated once")
        else:
            AutoI17nAgent._instantiated = True

        setattr(
            AsyncWebhookAdapter,
            "create_interaction_response",
            i17n_Adapter_create_interaction_response,
        )
        setattr(
            AsyncWebhookAdapter,
            "execute_webhook",
            i17n_AsyncWebhookAdapter_execute_webhook,
        )
        if handle_webhooks:
            setattr(Webhook, "send", wrap_i17n_sender(Webhook_send))
        setattr(Messageable, "send", wrap_i17n_sender(Messageable_send))
        setattr(
            InteractionResponse,
            "send_message",
            wrap_i17n_sender(InteractionResponse_send_message),
        )
        setattr(HTTPClient, "send_message", i17n_HTTPClient_send_message)
        setattr(Message, "edit", wrap_i17n_editer(Message_edit))
        setattr(
            InteractionResponse,
            "edit_message",
            wrap_i17n_editer(InteractionResponse_edit_message),
        )
        setattr(WebhookMessage, "edit", wrap_i17n_editer(WebhookMessage_edit))
        setattr(HTTPClient, "edit_message", i17n_HTTPClient_edit_message)
        setattr(InteractionResponse, "send_modal", i17n_InteractionResponse_send_modal)
    ...
    def get_config():
        ...
        """
        return {
            "source_lang": AutoI17nAgent.source_lang,
            "translator": AutoI17nAgent.translator,
            "translate_components": (
                AutoI17nAgent.translate_buttons
                or AutoI17nAgent.translate_selects
                or AutoI17nAgent.translate_modals
            ),
            "translate_messages": AutoI17nAgent.translate_messages,
            "translate_embeds": AutoI17nAgent.translate_embeds,
            "translate_buttons": AutoI17nAgent.translate_buttons,
            "translate_selects": AutoI17nAgent.translate_selects,
            "translate_modals": AutoI17nAgent.translate_modals,
        }
