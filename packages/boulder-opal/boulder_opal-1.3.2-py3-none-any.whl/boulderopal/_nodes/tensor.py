...
def _embed_tuple(term: Any, *, name: str) -> Tuple[Union[np.ndarray, Tensor], int]:
    Checker.TYPE(
        isinstance(term, (list, tuple)) and len(term) == 2, _embed_messenger(name)
    )

    operator, position = term
    return (
...
class TensorGraph:  # pylint: disable=too-many-public-methods,unused-argument
    ...
    def expectation_value(
        ...
        *,
        ...
        shape = validate_broadcasted_shape(
            state.shape[:-1], operator.shape[:-2], "state batch", "operator batch"
        )
    ...
    def inner_product(
        ...
        *,
        ...
        shape = validate_broadcasted_shape(
            x.shape[:-1], y.shape[:-1], "x batch", "y batch"
        )
    ...
    def density_matrix_infidelity(
        ...
        *,
        ...
        broadcasted_shape = validate_broadcasted_shape(
            x.shape[:-2], y.shape[:-2], "x batch", "y batch"
        )
    ...
    def embed_operators(
        ...
        *,
        ...
        batch_shape = get_broadcasted_shape(*[i[0].shape[:-2] for i in _operators])

        Checker.VALUE(
            batch_shape is not None,
            "The batch shapes of all the operators must be broadcastable.",
        )
    ...
    def kronecker_product_list(
        ...
        *,
        ...
        """
        batch_shape = get_broadcasted_shape(*[op.shape[:-2] for op in operators])
        Checker.VALUE(
            batch_shape is not None,
            "The batch shapes of the operators must be broadcastable.",
        )

        value_shape = (
            np.prod([op.shape[-2] for op in operators]),
            np.prod([op.shape[-1] for op in operators]),
        )
