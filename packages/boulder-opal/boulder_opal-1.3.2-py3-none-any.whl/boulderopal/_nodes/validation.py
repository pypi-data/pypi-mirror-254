...
def _get_matrix_shape(value: Any, name: str) -> tuple[tuple[int, ...], tuple[int, ...]]:
    def _validate_shape(x, strict=False):
        if strict:
...
def _get_vector_shape(value: Any, name: str) -> tuple[tuple[int, ...], tuple[int, ...]]:
    def _validate_shape(shape_, strict=False):
...
def _get_signal_shape(value: Any, name: str) -> tuple[tuple[int, ...], tuple[int, ...]]:
    def _validate_shape(shape_):
...
class _ShapeT:
    """
    A validator class to check shapes.

    Note that this validator only checks shape of the operator.
    """

    def __init__(self, type_: ShapeT):
        self._type = type_
        self._name = ""
        self._func_pipe = []
        self._value_dim = 0
