...
def pmatmat(L, method='thread', nworkers=None, use_matvec=False, **kwargs):
    ...
    of ``L``. Precisely, ``L.matmat``/``__matmul__`` is parallelized according to
    ...
    Args:
        ...
        use_matvec: (bool)
            If ``True`` the ``matvec`` function of ``L`` is used for parallelization otherwise
            only ``matmat`` is used.
    ...
    .. Warning:: For using ``pmatmat`` efficiently (i.e. without
        ...
        is to use the `threadpoolctl <https://pypi.org/project/threadpoolctl/>`_
        library.

    Returns:
        A specialized :py:class:`.LazyLinearOp` that is able to compute the product ``L @
        A`` in parallel according to the chosen method.
    ...
    Example:
        ...
        >>> pL = pmatmat(L) # pL matmat is parallelized using default thread method
