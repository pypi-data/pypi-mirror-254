...
class MPILop(LazyLinearOp):
    ...
    **See also**: :py:func:`.pmatmat`.
    ...
    def __init__(self, shape, mat_npz, mat_dtype, bcast_op=True):
        ...
        Args:
            shape:
                ...
                it is needed because only 0-rank MPI process will read the file).
            mat_npz:
    ...
    def _init_distributed_mat(self, axis=0, conj=False):
        self.Ls[axis], self.n_per_slot[axis], self.slot_inds[axis], sendcounts = \
                scatter_mat(self.comm, self.L, self.dtype, self.shape, axis=axis, conj=conj)
...
#                  ],
                  Ms)
    return Ms.conj() if axis == 1 and conj == True else Ms, n_per_slot, slot_inds, sendcounts
