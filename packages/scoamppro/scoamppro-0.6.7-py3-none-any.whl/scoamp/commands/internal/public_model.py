...
def auto_sync(
    src_dir: Path = typer.Argument(..., exists=True, file_okay=False, resolve_path=True, help='origin data directory'),
    work_dir: Path = typer.Argument(..., exists=False, file_okay=False, resolve_path=True, help='working directory'),
    allow_list: Path = typer.Argument(..., exists=True, dir_okay=False, resolve_path=True, help='allowed model list file'),
    ignore_list: Path = typer.Option(None, '-il', '--ignore-list', exists=True, dir_okay=False, resolve_path=True, help='ignored model list file'),
    sync_num: int = typer.Option(DEFAULT_AUTO_SYNC_NUM, '-num', '--sync-num', help='sync model number, default number is 41'),
    ):
    """
    structure of 'src_dir':
       ampftp
...
def _gen_auto_sync_list(manifest: list, allow_list: Path=None, ignore_list: Path=None, sync_num: int=DEFAULT_AUTO_SYNC_NUM):
    candicate = {item['key'] for item in manifest}
    plan_set = set()
    ...
    default_num = len(plan_set) if len(plan_set) < DEFAULT_AUTO_SYNC_NUM else DEFAULT_AUTO_SYNC_NUM
