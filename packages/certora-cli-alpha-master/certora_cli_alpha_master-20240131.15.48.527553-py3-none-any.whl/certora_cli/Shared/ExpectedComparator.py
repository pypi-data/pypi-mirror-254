...
class ExpectedComparator:
    ...
    def _extract_nested_rules_status(nested_rule: Dict[str, List[str]]) -> str:
        ...
        for _, value in nested_rule.items():
            if isinstance(value, str):
                ...
                return ExpectedComparator._extract_nested_rules_status(value)
                
        return 'SUCCESS'  # Return 'SUCCESS' only if no other condition is met.
    ...
    def __check_parametric_rule(rule: dict) -> str:
        ...
        return next((s for s in statuses if s in rule and rule[s]), '')