...
def anomaly_detection(series: 'list' = [], contamination=0.1,max_value=2) -> '[x, y]':
    scores = [
    ...
    return [series[r[0]] for r in result[:min(len(result),max_value)]]
...
def isolation_forest_anomaly_detection(series: 'list' = [], n_estimators=100, max_samples='auto', contamination=0.1,
    ...
    return pd.DataFrame({'index':[i for i in range(len(anomaly_score)) if anomaly_score[i]==1], 'x': series[anomaly_score == 1][:, 0], 'value': series[anomaly_score == 1][:, 1],
