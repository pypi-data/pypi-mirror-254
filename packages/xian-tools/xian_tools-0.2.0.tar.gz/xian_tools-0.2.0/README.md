...
```python
from xian-tools import Wallet, Xian
...
```python
from xian-tools import Wallet, Xian
from xian-tools.transaction import get_nonce, create_tx, broadcast_tx
...
broadcast = broadcast_tx(node_url, tx)
print(f'broadcast: {broadcast}')
print(f'next nonce: {get_nonce(node_url, wallet.public_key)}')
print(f'balance: {xian.get_balance()}')

tx_hash = broadcast["result"]["hash"]
