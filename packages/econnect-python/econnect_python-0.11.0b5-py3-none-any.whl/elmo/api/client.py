...
class ElmoClient:
    ...
    def __init__(self, base_url=None, domain=None, session_id=None):
        ...
        self._lock = Lock()
        # Debug
        _LOGGER.debug(f"Client | Version: {__version__}")
    ...
    def lock(self, code, user_id=1):
        ...
        _LOGGER.debug(f"Client Triage | Parametri LOCK: {response.request.body}")
        _LOGGER.debug(f"Client Triage | Risposta LOCK: {response.text}")
    ...
    def unlock(self):
        ...
        _LOGGER.debug(f"Client Triage | Parametri UNLOCK: {response.request.body}")
        _LOGGER.debug(f"Client Triage | Risposta UNLOCK: {response.text}")
        response.raise_for_status()
    ...
    def arm(self, sectors=None):
        ...
        _LOGGER.debug(f"Client Triage | Parametri ARM: {response.request.body}")
        _LOGGER.debug(f"Client Triage | Risposta ARM: {response.text}")
        response.raise_for_status()
        body = response.json()
    ...
    def disarm(self, sectors=None):
        ...
        response = self._session.post(self._router.send_command, data=payload)
        _LOGGER.debug(f"Client Triage | Parametri DISARM: {response.request.body}")
        _LOGGER.debug(f"Client Triage | Risposta DISARM: {response.text}")
        response.raise_for_status()
        body = response.json()
