        ...
        while True:
            ...
            if re.match(r'Program started: +{"\d+":"hubstudio_connector.exe"', stdstr):
                ...
                if self.echo:
                    print("即将自动退出......")
                break
            if re.match(r'.*?starting http server at:', stdstr):
                api_url = re.search(r"(\d+\.\d+\.\d+\.\d+:\d+)", stdstr).group(1)
            elif stdstr == 'Startup complete':
                break
        return api_url
...
class HubStudioClientAsync:
    ...
    async def get_environment_groups(self) -> list[dict]:
        """
        获取环境分组列表

        :return: list[dict], 分组名称和分组ID
        """
        response = await self.__request("/api/v1/group/list", method="POST")
        return response.get("data", [])

    async def create_environment_group(self, tag_name: str) -> bool:
        """
        新建环境分组
        ...
        response = await self.__request("/api/v1/group/create", payload, method="POST")
        return response.get("data")

    async def delete_environment_group(self, tag_code: str) -> bool:
        """
        删除环境分组
        ...
        await self.__request("/api/v1/group/del", payload, method="POST")