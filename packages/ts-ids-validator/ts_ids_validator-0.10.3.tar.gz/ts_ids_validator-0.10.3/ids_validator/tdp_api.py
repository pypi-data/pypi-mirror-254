...
class APIConfig:
    ...
    config_keys: ClassVar[List[str]] = ["api_url", "org", "auth_token"]
    api_url: str
    org: str
    auth_token: str
    ...
    def from_json_or_env(
        ...
        config_from_env = {key: get_env_value(key) for key in cls.config_keys}
    ...
    def validate_input(
        ...
        missing_keys = {
            ...
            if key not in api_config or api_config[key] is None
        }
        if missing_keys:
            raise ValueError(f"Configuration values not set for: {missing_keys}.")
        return cls(**api_config, source=source)

    def __str__(self):
        # Reveal same amount of auth_token as ts-sdk
        return f'APIConfig(api_url="{self.api_url}", org="{self.org}", auth_token="{self.auth_token[:7]}...", source="{self.source}")'
...
class IdsArtifactFile(str, Enum):
...
def get_ids_artifact(
    ...
    for file_slug in (
        ...
        # The API specifies that the actual file content under a key "content"
