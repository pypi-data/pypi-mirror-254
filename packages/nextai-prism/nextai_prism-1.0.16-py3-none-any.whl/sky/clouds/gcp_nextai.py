...
class GCPNextAI(clouds.Cloud):
    ...
    def get_current_user_identity(cls, credentials_path: Optional[str] = None, access_token: Optional[str] = None) -> Optional[List[str]]:
        ...
        if os.environ.get('CLOUDSDK_AUTH_EMAIL', None) is not None and os.environ.get('CLOUDSDK_CORE_PROJECT', None) is not None:
            return  [f'{os.environ["CLOUDSDK_AUTH_EMAIL"]} [project_id={os.environ["CLOUDSDK_CORE_PROJECT"]}]']
        try:
