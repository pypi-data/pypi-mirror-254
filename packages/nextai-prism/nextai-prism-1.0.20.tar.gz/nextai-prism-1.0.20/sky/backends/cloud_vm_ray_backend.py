...
class CloudVmRayBackend(backends.Backend['CloudVmRayResourceHandle']):
    ...
    def _sync_workdir(self, handle: CloudVmRayResourceHandle,
        ...
        def _sync_workdir_node(runner: command_runner.SSHCommandRunner) -> None:
            # try:
            #     runner.run("apt-get update && apt-get install -y rsync", log_path=log_path,)
            # except:
            #     pass
            runner.rsync(
                source=workdir,
    ...
    def _setup(self, handle: CloudVmRayResourceHandle, task: task_lib.Task,
        ...
        with tempfile.NamedTemporaryFile('w', prefix='sky_setup_') as f:
            ...
            def _setup_node(runner: command_runner.SSHCommandRunner) -> None:
                # try:
                #     runner.run("apt-get update && apt-get install -y rsync")
                # except:
                #     pass
    ...
    def _exec_code_on_head(
        ...
        if isinstance(handle.launched_resources.cloud, clouds.Local):
            ...
            job_submit_cmd = (
                ...
                f'{cd} && ray job submit '
                '--address=http://0.0.0.0:$RAY_DASHBOARD_PORT '
    ...
    def _setup_and_create_job_cmd_on_local_head(
        ...
        job_submit_cmd = (
            'ray job submit '
            '--address='
            f'http://0.0.0.0:{constants.SKY_REMOTE_RAY_DASHBOARD_PORT} '
