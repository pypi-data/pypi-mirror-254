...
def merge_env(env_pack, other_pack):
    """
    Merge the environment package with another package.
    """
    for var_dict in other_pack:
        name = var_dict['name']
        value = var_dict['value']
        merge_policy = var_dict['merge_policy']

        if name in env_pack['environment']:
            if merge_policy == 'exclusive':
                # Overwrite the existing value
                env_pack['environment'][name] = value
            elif merge_policy == 'append':
                # Append the new value to the existing one, separated by a colon
                existing_value = env_pack['environment'][name]
                env_pack['environment'][name] = existing_value + ':' + value
        else:
            # If the key is not in env_pack, add it to the 'environment'
            env_pack['environment'][name] = value
