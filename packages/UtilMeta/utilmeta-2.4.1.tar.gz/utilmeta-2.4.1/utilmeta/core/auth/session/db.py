...
class DBSessionSchema(BaseSessionSchema):
    ...
    def load_object(self, must_create: bool = False):
        ...
        if not self.session_key:
            ...
            obj = self._model_cls.objects.filter(session_key=self.session_key).first()
    ...
    async def adb_delete(self, session_key=None):
        if session_key is None:
            if self.session_key is None:
                return
            session_key = self.session_key
        await self._model_cls.objects.filter(
            session_key=session_key, deleted_time=None).aupdate(deleted_time=time_now())
    ...
    def load(self):
        if not self._session_key:
            return None
        # to be inherited
        session = self._model_cls.objects.filter(
