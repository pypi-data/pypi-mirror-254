...
from kitoboy_optimizator.enums import Strategies, Exchangies
...
class MultyOptimizer:
    ...
    def sync_wrapper_execute_optimization_task(self, task):
        ...
        """
        try:
            print(f"START optimization {task.strategy.value} {task.symbol} {task.interval} {task.exchange.value}")
            result = asyncio.run(self.__execute_optimization_task(task))
            
        except Exception as e:
            print(f"FAILED to execute optimization task: {e}\n {task.strategy.value} {task.symbol} {task.interval} {task.exchange.value}\n")
        return result
