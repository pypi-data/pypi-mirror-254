...
from predibase.util import log_error, log_warning
    ...
    if server_release_version is not None:
        if server_release_version == "" or server_release_version == "staging":
            if not __version__.startswith("0.1.1+dev") and not __version__.startswith("v2999"):
                log_warning("WARN: Using a post-release / prod version of the SDK in staging can lead to unexpected "
            ...
            if sdk_semver < server_release_semver:
                log_warning(f"WARN: Currently installed SDK is outdated. This can lead to bugs or unexpected behavior. "
