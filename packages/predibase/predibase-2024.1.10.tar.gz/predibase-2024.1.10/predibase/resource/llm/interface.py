...
from lorax.types import StreamResponse as LoraxStreamResponse
...
class LLMDeployment:
    def __init__(
        ...
        self.lorax_client = LoraxClient(
            base_url=self._get_deployment_url(),
    ...
    def generate(self, prompt: str, options: Optional[Dict[str, Union[str, float]]] = None) -> LoraxResponse:
        options = self._validate_generate_options(options)
        return self.lorax_client.generate(prompt=prompt, **options)

    def generate_stream(
        self,
        prompt: str,
        options: Optional[Dict[str, Union[str, float]]] = None,
    ) -> Iterator[LoraxStreamResponse]:
        options = self._validate_generate_options(options)
    ...
    def generate_batch(
        ...
        return self._execute_in_threadpool(self.generate, prompts, options=options)
    ...
    def prompt_batch(
        ...
        return self._execute_in_threadpool(
            self.prompt,
            data_list,
    ...
    def _get_deployment_url(self):
        return f"http://{self.session.serving_http_endpoint}/{self.session.tenant}/deployments/v2/llms/{self.name}"
    ...
    def _execute_in_threadpool(
        ...
        **kwargs,
        ...
        """Executes a given function in a ThreadPoolExecutor.

        :param func: The function to be executed.
        :param items: A list of items to be processed by the function.
    ...
    def _validate_generate_options(self, options: Dict[str, Union[str, float]]):
        adapter_id, adapter_source = options.get("adapter_id", None), options.get("adapter_source", None)
        if not (adapter_id is None and adapter_source is None):
            raise ValueError("Cannot have `adapter_id` or `adapter_source` be part of the `options` parameter.")

        options = options if isinstance(options, dict) else dict()
        options["adapter_id"], options["adapter_source"] = None, None
        if self._adapter:
...
def deploy_llm(
    ...
    auto_suspend_seconds = 3600 if auto_suspend_seconds == None else auto_suspend_seconds
