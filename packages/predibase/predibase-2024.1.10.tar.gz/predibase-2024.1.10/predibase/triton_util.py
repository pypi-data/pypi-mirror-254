...
def triton_metadata_http(endpoint, model_name, headers, model_version=""):
    # FUTURE: Make HTTPS (ssl=True, ssl_options={"ca_certs": secure_cacert, "cert_reqs": ssl.CERT_REQUIRED})
    with httpclient.InferenceServerClient(endpoint) as triton_client:
...
async def triton_metadata_http_async(endpoint, model_name, headers, model_version=""):
    # FUTURE: Make HTTPS (ssl=True, ssl_options={"ca_certs": secure_cacert, "cert_reqs": ssl.CERT_REQUIRED})
    async with aio_httpclient.InferenceServerClient(endpoint) as triton_client:
