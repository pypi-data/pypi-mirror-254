...
def test_infer_datatype():
    ...
    # sql
    assert pySteve.infer_datatype(123, 'SQL')   == ('TINYINT', 123)
    assert pySteve.infer_datatype('123', 'SQL') == ('TINYINT', 123)
    assert pySteve.infer_datatype(12345, 'SQL')   == ('SMALLINT', 12345)
    assert pySteve.infer_datatype(123456, 'SQL')   == ('INTEGER', 123456)
    assert pySteve.infer_datatype(123456789, 'SQL')   == ('INTEGER', 123456789)
    assert pySteve.infer_datatype(12345678901, 'SQL')   == ('BIGINT', 12345678901)    
    assert pySteve.infer_datatype(1234567890123456789, 'SQL')   == ('BIGINT', 1234567890123456789)    
    assert pySteve.infer_datatype(123.456, 'SQL')   == ('DECIMAL(6,3)', 123.456)
    assert pySteve.infer_datatype('123.456', 'SQL') == ('DECIMAL(6,3)', 123.456)
    assert pySteve.infer_datatype('123456789.123', 'SQL') == ('DECIMAL(12,3)', 123456789.123)
    assert pySteve.infer_datatype('toy boat', 'SQL')   == ('VARCHAR(8)', 'toy boat' )
    assert pySteve.infer_datatype('"toy boat"', 'SQL') == ('VARCHAR(8)', 'toy boat' )
    assert pySteve.infer_datatype('[1, 3, 5, "seven"]', 'SQL') == ('VARCHAR(18)', [1, 3, 5, 'seven'])
    assert pySteve.infer_datatype('2024-12-31', 'SQL') == ('DATE', datetime(2024, 12, 31, 0, 0))
    assert pySteve.infer_datatype('1974-10-15', 'SQL') == ('DATE', datetime(1974, 10, 15, 0, 0))
    assert pySteve.infer_datatype('1974-15-10', 'SQL') == ('VARCHAR(10)', '1974-15-10')
    assert pySteve.infer_datatype('2024-09-10 11:12:13', 'SQL') ==('TIMESTAMP', datetime(2024, 9, 10, 11, 12, 13))
    assert pySteve.infer_datatype('2024-08-15 23:59:59.999999', 'SQL') ==('TIMESTAMP', datetime(2024, 8, 15, 23, 59, 59, 999999))
    assert pySteve.infer_datatype('2024-08-15 23:59:59.999999+08:00', 'SQL') ==('TIMESTAMP', datetime(2024, 8, 15, 23, 59, 59, 999999, tzinfo=timezone(timedelta(seconds=28800))))
