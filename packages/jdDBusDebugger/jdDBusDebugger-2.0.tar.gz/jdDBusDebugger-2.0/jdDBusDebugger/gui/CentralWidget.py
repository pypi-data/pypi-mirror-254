...
from ..Functions import clear_table_widget
...
class CentralWidget(QWidget, Ui_CentralWidget):
    def __init__(self, env: "Environment", main_window: "MainWindow", connection: "Connection") -> None:
        ...
        self._update_objects_buttons_endabled()
    ...
    def _update_service_table(self) -> None:
        ...
        for row, service in enumerate(self.connection.service_list):
            ...
            self.services_table.setItem(row, _ServicesTableColumns.PID, pid_item)
    ...
    def _update_objects_buttons_endabled(self) -> None:
    ...
    def call_method(self, method: "Method", arguments: list["DBusValue"], record_macro: bool) -> None:
        ...
        if method.return_type is not None and method.return_type.type_const == DBusTypeEnum.OBJECT_PATH:
            ...
            self.add_log_text(f"object_path<{object_path}>")
            return

        match len(result.arguments()):
            case 0:
                self.add_log_text(QCoreApplication.translate("CentralWidget", "No return value"))
            case 1:
                self.add_log_text(str(result.arguments()[0]))
            case _:
                self.add_log_text(str(result.arguments()))
    ...
    def connect_signal(self, signal: "Signal", record_macro: bool) -> None:
        ...
        if not signal.is_connected:
            self.connection.connection.connect(signal.interface.service.name, signal.interface.object_path, signal.interface.name, signal.name, self._dbus_signal_callback)
    ...
    def _dbus_signal_callback(self, msg: QDBusMessage) -> None:
        self.add_log_text(QCoreApplication.translate("CentralWidget", "Signal {{signal}} emited: {{reply}}").replace("{{signal}}", msg.member()).replace("{{reply}}", str(msg.arguments())))
