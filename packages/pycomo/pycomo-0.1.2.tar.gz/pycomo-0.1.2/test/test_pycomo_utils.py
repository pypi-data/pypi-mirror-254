...
def test_get_model_biomass_compound_wrong_id():
    with pytest.raises(AssertionError):
        pycomo.get_model_biomass_compound(model, expected_biomass_id="a", generate_if_none=True)


def test_get_model_biomass_compound_correct_id():
    biomass_met_id = "cpd11416_c0"
    biomass_met = pycomo.get_model_biomass_compound(model, expected_biomass_id=biomass_met_id, generate_if_none=True)
    assert biomass_met.id == biomass_met_id


def test_get_model_biomass_compound_no_id_no_generate():
    with pytest.raises(AssertionError):
        biomass_met = pycomo.get_model_biomass_compound(model, generate_if_none=False)


def test_get_model_biomass_compound_no_id_generate():
    biomass_met = pycomo.get_model_biomass_compound(model, generate_if_none=True)
    assert biomass_met.id == "cpd11416_exchg"
...
def test_get_model_biomass_compound_expected_in_objective():
    with model as multi_model:
        ...
        biomass_met = pycomo.get_model_biomass_compound(model, expected_biomass_id="cpd11416_c0", generate_if_none=True)
