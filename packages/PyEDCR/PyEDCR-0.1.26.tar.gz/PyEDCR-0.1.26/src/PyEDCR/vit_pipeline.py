...
def get_and_print_metrics(fine_predictions: np.array,
    ...
    test_coarse_f1 = f1_score(y_true=true_coarse_data,
                              ...
                              average='macro')
    test_fine_recall = recall_score(y_true=true_fine_data,
                                        y_pred=fine_predictions,
                                        average="macro")
    test_coarse_recall = recall_score(y_true=true_coarse_data,
                                          y_pred=coarse_predictions,
                                          average="macro")
    test_fine_precision = precision_score(y_true=true_fine_data,
                                        y_pred=fine_predictions,
                                        average="macro")
    test_coarse_precision = precision_score(y_true=true_coarse_data,
                                          y_pred=coarse_predictions,
                                          average="macro")
    ...
    print((f'Main model name: {utils.blue_text(model_name)} ' if model_name != '' else '') +
          ...
          f' fine-grain {prior_str} {combined_str} average recall: {utils.green_text(round(test_fine_recall * 100, 2))}%'
          f' fine-grain {prior_str} {combined_str} average precision: {utils.green_text(round(test_fine_precision * 100, 2))}%'
          ...
          f', coarse-grain {prior_str} {combined_str} average recall: '
          f'{utils.green_text(round(test_coarse_recall * 100, 2))}%'
          f', coarse-grain {prior_str} {combined_str} average precision: '
          f'{utils.green_text(round(test_coarse_precision * 100, 2))}%')
