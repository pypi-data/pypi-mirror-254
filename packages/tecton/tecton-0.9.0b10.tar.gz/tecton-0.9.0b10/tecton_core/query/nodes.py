...
from tecton_core.query.custom_functions import RowNumber
...
class TakeLastRowNode(QueryNode):
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        row_num_query = (
            ...
            .with_(input_query, aliased_input)
            .from_(AliasedQuery(aliased_input))
            .select("*", row_num.as_("row_num"))
        )

        query = self.func.query().from_(row_num_query).select(*fields).where(Field("row_num") == 1)
