...
class NieuwbouwprojectspiderSpider(scrapy.Spider):
    ...
    def get_to_project_page (self,response):
    ...
    def get_to_sell_project_page(self,resposne):
        ...
        yield scrapy.Request (url = 'https://www.nieuwbouw-nederland.nl/projecten/?fs=&hk[]=k&sort=',callback = self.parse)
    
    

    def parse(self, response):

            projects = response.css('section#list article a::attr(href)').getall()
            ...
            for project in projects:
                ...
                if project:
                    ...
                    yield response.follow(project, callback=self.parse_project_page,cb_kwargs={'projects':projects,'project':project,'project_info':project_info,'project_list':project_list})

            next_page_link = response.xpath('//a[contains(text(), "volgende")]/@href').get()

            if next_page_link and project_count == len(projects):
