...
class Window:
    ...
    def new_entry(self, posy, posx, width, fg, placeholder=""):
        ...
        entry.insert(0, placeholder)  # Insert initial text in the field if provided
        return entry
    ...
    def new_submenu(self, menu, label):
        submenu = tk.Menu(menu, tearoff=0)
        menu.add_cascade(label=label, menu=submenu)
        return submenu

    def new_submenu_button(self, submenu, text, command=None):
        submenu.add_command(label=text, command=command)
