...
from sqlglot.optimizer.scope import find_all_in_scope, walk_in_scope, traverse_scope
...
def propagate_filters(expression, root=False):
    if isinstance(expression, exp.Select) and (root or not expression.same_parent):
        constant_mapping = {}
        for scope in traverse_scope(expression):
            for expr, *_ in walk_in_scope(
                scope.expression, prune=lambda node, *_: isinstance(node, exp.If)
            ):
...
def _simplify_comparison(expression, left, right, or_=False):
    if isinstance(left, COMPARISONS) and isinstance(right, COMPARISONS):
        ...
        if matching and columns:
            try:
                left = first(largs - columns)
                right = first(rargs - columns)
            except StopIteration:
                return expression

            # make sure the comparison is always of the form x > 1 instead of 1 < x
            if left.__class__ in INVERSE_COMPARISONS and left == ll:
                left = INVERSE_COMPARISONS[left.__class__](this=lr, expression=ll)
            if right.__class__ in INVERSE_COMPARISONS and right == rl:
...
def propagate_constants(expression, root=True):
    ...
    if (
        ...
        constant_mapping = {}
        for expr, *_ in walk_in_scope(
            expression, prune=lambda node, *_: isinstance(node, exp.If)
        ):
            if isinstance(expr, exp.EQ):
                left, right = expr.left, expr.right
...
def simplify_equality(expression: exp.Expression) -> exp.Expression:
    ...
    if isinstance(expression, COMPARISONS):
        ...
        if left.__class__ in INVERSE_OPS:
            pass
        elif right.__class__ in INVERSE_OPS:
            left, right = right, left
        else:
            return expression
        ...
        elif _is_date_literal(right):
            a_predicate = _is_date_literal
            b_predicate = _is_interval
        else:
            return expression

        if left.__class__ in INVERSE_DATE_OPS:
            left = t.cast(exp.IntervalOp, left)
            a = left.this
            b = left.interval()
        else:
            left = t.cast(exp.Binary, left)
        ...
        return expression.__class__(
            this=a, expression=INVERSE_OPS[left.__class__](this=right, expression=b)
        )
    return expression
...
DATETRUNC_BINARY_COMPARISONS: t.Dict[
    ...
    exp.LT: lambda left, date, unit: left < date_literal(date_floor(date, unit)),
    exp.GT: lambda left, date, unit: (
        left >= date_literal(date_floor(date, unit) + interval(unit))
    ),
    exp.LTE: lambda left, date, unit: (
        left < date_literal(date_floor(date, unit) + interval(unit))
    ),
    exp.GTE: lambda left, date, unit: left >= date_literal(date_ceil(date, unit)),
...
def _is_datetrunc_predicate(left: exp.Expression, right: exp.Expression) -> bool:
    return isinstance(left, (exp.DateTrunc, exp.TimestampTrunc)) and _is_date_literal(
        right
    )
...
def simplify_datetrunc_predicate(expression: exp.Expression) -> exp.Expression:
    ...
    if isinstance(expression, exp.Binary):
        ...
        if _is_datetrunc_predicate(left, right):
            pass
        elif _is_datetrunc_predicate(right, left):
            comparison = INVERSE_COMPARISONS.get(comparison, comparison)
            left, right = right, left
        else:
            return expression

        left = t.cast(exp.DateTrunc, left)
        unit = left.unit.name.lower()
        date = extract_date(right)

        if not date:
            return expression

        return (
            DATETRUNC_BINARY_COMPARISONS[comparison](left.this, date, unit)
            or expression
        )
        ...
        if rs and all(_is_datetrunc_predicate(left, r) for r in rs):
            left = t.cast(exp.DateTrunc, left)
            unit = left.unit.name.lower()
            ...
            return exp.or_(
                *[_datetrunc_eq_expression(left, drange) for drange in ranges],
                copy=False,
            )
