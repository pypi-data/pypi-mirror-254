...
class Api:
    ...
    async def fetch_async(self, **kwargs: Any) -> "Api":
        ...
        for attempt in range(self.retries_max_retries):
            try:
                ...
                self.log.warning(
                    f"Retrying API call. Attempt: {attempt + 1} of {self.retries_max_retries}. Error: {e}"
                )
                if attempt == self.retries_max_retries - 1:
                    self.log.error(
                        f"Attempted max times {self.retries_max_retries}"
                    )
                    raise e
                await asyncio.sleep(
                    self.retries_initial_delay * (self.retries_backoff_multiplier ** attempt)
                )
    ...
    def python_object(self, **kwargs: Any) -> "Api":
        ...
        try:
            ...
            self.log.warning(
                f"Data is not in JSON format. Carrying on, setting to []"
            )
            self.data = []
