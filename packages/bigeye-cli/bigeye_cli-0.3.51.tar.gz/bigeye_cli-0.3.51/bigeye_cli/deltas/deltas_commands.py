...
def suggest_deltas(
    ...
    if len(target_selector) == 2:
        ...
        matching_target_tables = (
            lineage_controller.infer_relationships_from_lists_of_tables(
...
def run_delta(
    ...
    if await_results:
        ...
        table.add_column(f"{delta.name} \nrun at {strftime('%Y-%m-%d %H:%M:%S', localtime(delta.last_run_at_epoch_seconds))}",
                         justify="right", style="cyan", width=50)
        table.add_column(justify="right", style="green")
        table.add_row("Total Metrics", str(delta.total_metric_count))
        table.add_row("Healthy Metrics", str(delta.total_metric_count - delta.failed_metric_count - delta.alerting_metric_count))
