...
class Popen:
    ...
    def _bytes_lines(self, source: Union[bytes, str]) -> List[bytes]:
        """Return a list of byteslines from bytes or str source"""
    ...
    def communicate(
        ...
        if self._stdout_target is not None:
            ...
            if stdout is not None:
                stdout_lines.extend(self._bytes_lines(stdout))
            #
            if self._stderr_target == subprocess.STDOUT and stderr is not None:
                stdout_lines.extend(self._bytes_lines(stderr))
            #
            output_stdout = b"\n".join(stdout_lines)
        #
        if self._stderr_target == subprocess.PIPE and stderr is not None:
