    ...
    for module in project.modules.values():
        for processor in module.processors.values():
            ...
            if version_info != None:
                ...
                # todo упустил часть пути, /configuration доработать там где определяется processor.defined_in_file
                link = f"{repo}/-/blob/{version_info['branch']}/configuration/{processor.defined_in_file}"
                info.update(
                    {
                        "source_link": link,
                    }
