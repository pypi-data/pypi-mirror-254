...
class Discussion():
    def __init__(self, answers_file, questions, has_drive, submit_url, net_id):
        self.answers_file = answers_file if not has_drive else f"/content/drive/MyDrive/CS270/{answers_file}"
        self.net_id = net_id  # Dynamic net_id passed during instantiation
    ...
    def submit_answers(self):
        ...
        answers = [box.children[1].value for box in self.boxes]
        data = {
            "net_id": self.net_id,  # Use the dynamic net_id
            "answers": answers,
            "question": self.answers_file
        }
        try:
