...
import robotgpt
from robotgpt import api_requestor, util
from robotgpt.robotgpt_response import RobotGPTResponse


class RobotGPTObject(dict):
    ...
    def __init__(
        ...
        **params,
        ...
        super(RobotGPTObject, self).__init__()
    ...
    def __setattr__(self, k, v):
        if k[0] == "_" or k in self.__dict__:
            return super(RobotGPTObject, self).__setattr__(k, v)
    ...
    def __delattr__(self, k):
        if k[0] == "_" or k in self.__dict__:
            return super(RobotGPTObject, self).__delattr__(k)
        else:
            del self[k]
    ...
    def __setitem__(self, k, v):
        ...
        super(RobotGPTObject, self).__setitem__(k, v)
    ...
    def refresh_from(
        ...
        self.api_token = api_token or getattr(values, "api_token", None)
        self._response_ms = response_ms or getattr(values, "_response_ms", None)

        # Wipe old state before setting new.
        self.clear()
        for k, v in values.items():
            super(RobotGPTObject, self).__setitem__(
                k, util.convert_to_openai_object(v, api_token)
            )
    ...
    def request(
        ...
        requestor = api_requestor.APIRequestor(
            ...
            api_url=self.api_url or self.api_url(),
            model=self.model
        )
        response, stream, api_token = requestor.request(
            method,
            params=params,
            stream=stream,
            headers=headers,
        ...
        if stream:
            assert not isinstance(response, RobotGPTResponse)  # must be an iterator
            return (
    ...
    async def arequest(
        ...
        response, stream, api_token = await requestor.arequest(
            method,
            params=params,
            stream=stream,
            headers=headers,
    ...
    def to_dict_recursive(self):
        d = dict(self)
        for k, v in d.items():
            if isinstance(v, RobotGPTObject):
                d[k] = v.to_dict_recursive()
            elif isinstance(v, list):
                d[k] = [
                    e.to_dict_recursive() if isinstance(e, RobotGPTObject) else e
                    for e in v
                ]
        return d

    @property
    def robotgpt_id(self):
        return self.id
    ...
    def __copy__(self):
        ...
        for k, v in self.items():
            ...
            super(RobotGPTObject, copied).__setitem__(k, v)
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in self.items():
            # Call parent's __setitem__ to avoid checks that we've added in the
            # overridden version that can throw exceptions.
            super(RobotGPTObject, copied).__setitem__(k, deepcopy(v, memo))
