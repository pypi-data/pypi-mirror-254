...
class GiftCardTestCase(CompanyTestMixin, ModuleTestCase):
    ...
    def test0600_gift_card_method(self):
        ...
        with set_company(company), \
            ...
            #################################
            ## invoice sequence on fiscalyear is lost in some strange way
            ## issue 5140
            #FiscalYear = pool.get('account.fiscalyear')
            #fys = FiscalYear.search([])
            #print(Transaction().context)
            #print(fys)
            #fiscalyear = FiscalYear.find(company.id, test_state=True)
            #print('fy', fiscalyear)
            #print('fy', fiscalyear.invoice_sequences)

            #from trytond.modules.account_invoice.tests import set_invoice_sequences
            #fiscalyear = set_invoice_sequences(fiscalyear)
            #fiscalyear.save()
            #fiscalyear, =  FiscalYear.browse([fiscalyear.id])
            #print('fy2', fiscalyear.rec_name, fiscalyear.invoice_sequences)


            #InvoiceSequence = pool.get('account.fiscalyear.invoice_sequence')
            #iseq = InvoiceSequence.search([()])
            #print('iseq', iseq)
            #print('iseq', [s.fiscalyear.rec_name for s in iseq])
            #######################################
