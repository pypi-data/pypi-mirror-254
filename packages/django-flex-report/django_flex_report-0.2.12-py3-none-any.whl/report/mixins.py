...
class TablePageMixin(PaginationMixin, TemplateObjectMixin):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.report_qs = self.template_filters.qs.distinct().filter(
            pk__in=[*map(attrgetter('pk'), self.quicksearch.qs.distinct() & self.filters.qs.distinct())]
        ...
        if self.template_object.model_user_path:
            try:
                ...
                if hasattr(model, app_settings.MODEL_USER_PATH_FUNC_NAME):
                    val = call(
                        getattr(model, app_settings.MODEL_USER_PATH_FUNC_NAME),
                        request=self.request,
                    )

                self.report_qs = self.report_qs.filter(
                    **{self.template_object.model_user_path: val}
                ).order_by(*model._meta.ordering or ["pk"])
            except FieldError:
                pass
