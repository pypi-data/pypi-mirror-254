...
class OCR:
    def __init__(self,detect=False,
                 tamil_model_path=os.path.join(current_path,"model_weights","parseq_tamil.onnx"),
                 eng_model_path=os.path.join(current_path,"model_weights","parseq_english.onnx"),
    ...
    def load_model(self):
        ...
        self.eng_character_set = r"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"
        self.tamil_character_set = r"~0123456789"
        self.img_transform = self.get_transform(self.img_size)

        # Tamil model
        self.tamil_parseq = self.initialize_onnx_model(self.tamil_model_path)
        self.tamil_tokenizer = Tokenizer(self.tamil_character_set)
    ...
    def text_recognize(self,img_org):
        ...
        img_org = self.img_transform(Image.fromarray(np.uint8(img_org)).convert('RGB')).unsqueeze(0)

        # Tamil Decode
        ort_inputs = {self.tamil_parseq.get_inputs()[0].name: self.to_numpy(img_org)}
        logits = self.tamil_parseq.run(None, ort_inputs)[0]
        probs = torch.tensor(logits).softmax(-1)
        label, confidence = self.tamil_tokenizer.decode(probs)
        ...
        print(eng_preds, eng_confidence)
        eng_preds = eng_preds[0]
...
if __name__ == "__main__":
    image_path = r"C:\Users\gnana\Documents\GitHub\tamil_ocr\test_images\0.jpg"
    ocr = OCR()
