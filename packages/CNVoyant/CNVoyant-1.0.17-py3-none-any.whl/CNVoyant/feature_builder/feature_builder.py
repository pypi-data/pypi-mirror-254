...
class FeatureBuilder:
    ...
    def __init__(self, variant_df: pd.DataFrame, data_dir: str, ref: str):
        ...
        self.gnomadSV = pd.read_csv(os.path.join(self.data_dir, 'gnomad4.csv.gz'), dtype = {'CHROM': str})
        self.gnomadSV['CHROM'] = self.gnomadSV['CHROM'].str[3:]
        self.gnomadSV = {chrom: self.gnomadSV[self.gnomadSV['CHROM'] == chrom] for chrom in self.gnomadSV['CHROM'].unique()}
    ...
    def get_features(self):#, clinvar_ids: list): 
        ...
        def get_hi_ts_regions(row):
            ...
            hi_ts_cols = ['NO_EVIDENCE','LITTLE_EVIDENCE','EMERGING_EVIDENCE',
                'SUFFICIENT_EVIDENCE','AUTOSOMAL RECESSIVE','UNLIKELY']
            ...
            #     df[f'adjusted_{col}'] = df['percent_coverage'] * df[col].astype(float)
        ...
        def get_population_frequency(row):
            ...
            gnomad_df = self.gnomadSV[row['CHROMOSOME']]

            gnomad_df = gnomad_df.loc[
            ...
            if not gnomad_df.empty:
                ...
                gnomad_df['percent_coverage'] = (
                gnomad_df[['START', 'END']].clip(lower=row['START'], upper=row['END']).diff(axis=1) / 
                            gnomad_df[['START', 'END']].diff(axis=1)
                            )['END']
        ...
        cnv_df.to_csv('/igm/projects/CNVoyant/CNVoyant/data/test.csv', index = False)
