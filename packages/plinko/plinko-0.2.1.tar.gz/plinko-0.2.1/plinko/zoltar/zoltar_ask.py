...
def zoltar_ask(wish):
    ...
    if (os.getenv("CONNECT_API_KEY") is None) or (os.getenv("CONNECT_USERNAME") is None): configure_username()

    # Check for connect server configuration
    if (os.getenv("CONNECT_SERVER") is None) or (os.getenv("CONNECT_SERVER_TYPE") is None): configure_environment()
