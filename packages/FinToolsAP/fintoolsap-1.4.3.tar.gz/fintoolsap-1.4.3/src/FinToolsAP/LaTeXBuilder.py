...
def df_to_tex_file(df: pd.DataFrame, 
                   output_path: str, 
                   date_col: str = 'date', 
                   **kwargs
                ) -> None:
...
def table_document(input_path: pathlib.Path, 
                   output_path: pathlib.Path = pathlib.Path('output/'), 
                   file_name: str = 'merged_document.tex'
                ) -> None:
    """
   ...
   # convert to pathlib.Path if not
    if(not isinstance(input_path, pathlib.Path)):
        if(isinstance(input_path, str)):
            ...
            raise TypeError('\'input_path\' must be a string or a \'pathlib.Path\' object.')
    ...
    written_output_file_path, is_file = _init_fs(input_path, output_path, file_name)
    
    with open(written_output_file_path, 'a') as output_file:
    ...
    _compile_document(written_output_file_path)
    
def graph_document(input_path: pathlib.Path, 
                   output_path: pathlib.Path = pathlib.Path('output/'), 
                   file_name: str = 'merged_document.tex'
                ) -> None:
    """
    ...
    written_file_path, is_file = _init_fs(input_path, output_path, file_name)
...
def _init_fs(input_path: pathlib.Path, 
    ...
    if(not os.path.exists(input_path)): 
        raise Exception(f'File path \'{input_path}\' does not exist.')
    ...
    # path to output file
    written_output_file_path = None
    if(output_path.is_absolute()):
        written_output_file_path = output_path
    else:
        ...
        i = 1
        curr_output_file_path = container_dir / output_path        
        while(os.path.exists(curr_output_file_path)):
            curr_output_file_path = container_dir / pathlib.Path(f'{output_path}{i}/')
            i += 1
        written_output_file_path = curr_output_file_path

    
    os.makedirs(written_output_file_path, exist_ok = True) 
    written_output_file_path /= file_name
    
    return(written_output_file_path, is_file)
