...
class MeterNeedSecurityAuthentication(Exception):
    ...
    def __str__(self):
        return "MeterOperationError - 电表需要安全认证, 表位:[{}].".format(repr(self.value))
...
class MeterSecurityAuthenticationError(Exception):
    ...
    def __str__(self):
        return "MeterLinkError - 电表安全认证失败, {}.".format(repr(self.value))
...
class BenchReadNoneError(Exception):
    ...
    def __str__(self):
        return "MeterOperationError - 表台读取误差错误, {}.".format(repr(self.value))
