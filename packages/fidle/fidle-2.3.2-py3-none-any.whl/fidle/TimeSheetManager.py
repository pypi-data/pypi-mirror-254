...
class TimeSheetManager:
    ...
    def __check_imap_status(self,status, response, message):
        '''
        Check imaplib return status. Raise an exception if not ok.
        '''
        if status != 'OK' : raise RuntimeError(f'{message}\nReponse was:\n{response[0]}')
    ...
    def get_presences( self, 
        ...
        #
        status, response = M.select(imap_box)
        self.__check_imap_status(status,response, f'Cannot found imap folder : [{imap_box}]')
        
        status, uids = M.uid('SEARCH', filter)
        self.__check_imap_status(status,uids,     f'IMAP search error with filter : [{filter}]')
        ...
        for uid in mail_uids:
            ...
            self.__check_imap_status(status,flags,     f'IMAP uid fetch flags error for uid : [{uid}]')
            ...
            self.__check_imap_status(status,data,     f'IMAP uid fetch message error for uid : [{uid}]')
